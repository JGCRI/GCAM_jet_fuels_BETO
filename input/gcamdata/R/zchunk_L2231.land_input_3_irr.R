...
#' \item{"L2231.LN3_HistUnmgdAllocation_noprot: Historical unmanaged land data from L2231.LN3_HistUnmgdAllocation is multiplied
#' by specified fraction to give unprotected land allocation in the third nest. OtherArableLands omitted."}
#' \item{"L2231.LN3_UnmgdAllocation_noprot: Unmanaged land data from L2231.LN3_UnmgdAllocation is multiplied by specified
#' fraction to give "unprotected land allocation in the third nest. OtherArableLands omitted."}
#' \item{"L2231.LN1_HistUnmgdAllocation_prot: Historical unmanaged land data from L2231.LN3_HistUnmgdAllocation is multiplied by
#' specified fraction to give protected land allocation in the first nest. OtherArableLands omitted."}
#' \item{"L2231.LN1_UnmgdAllocation_prot: Unmanaged land data from L2231.LN3_UnmgdAllocation is multiplied by specified fraction
...
module_aglu_L2231.land_input_3_irr <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU <- get_data(all_data, "L1251.PeatCarbonContent_kgm2_R_LT_C_GLU", strip_attributes = TRUE)
    ...
    if(aglu.CARBON_DATA_SOURCE=="moirai"){
      ...
      L121.CarbonContent_kgm2_R_LT_GLU <- get_data(all_data, "L120.LC_soil_veg_carbon_GLU", strip_attributes = TRUE)
    }else{
      L121.CarbonContent_kgm2_R_LT_GLU <- get_data(all_data, "L121.CarbonContent_kgm2_R_LT_GLU", strip_attributes = TRUE)
    ...
    # L2231.LN3_Logit: Logit exponent of the third nest.
    ...
    L125.LC_bm2_R_LT_Yh_GLU %>%
      ...
      L2231.LC_bm2_R_Unmgd3_Yh_GLU

    # L2231.LN3_HistUnmgdAllocation: Historical land cover, unmanaged land in the third nest
    # L2231.LN3_UnmgdAllocation: Model base period land cover, unmanaged land in the third nest
    ...
    L125.LC_bm2_R_LT_Yh_GLU %>%
      ...
      add_node_leaf_names(nesting_table = A_LandLeaf3, leaf_name = "LandLeaf",
                          LN1 = "LandNode1", LN2 = "LandNode2", LN3 = "LandNode3") ->
      L2231.LC_bm2_R_Mgd3_Yh_GLU

    # L2231.LN3_HistMgdAllocation_noncrop: Historical land cover, managed land in the third nest (noncrop)
    # L2231.LN3_MgdAllocation_noncrop: Model base period land cover, managed land in the third nest (noncrop)
    # Formed from filtering the master table by different years.
    L2231.LC_bm2_R_Mgd3_Yh_GLU %>%
      filter(year %in% aglu.LAND_HISTORY_YEARS) %>%
    ...
    # L2231.LN3_UnmgdCarbon: Carbon content info, unmanaged land in the third nest
    L2231.LC_bm2_R_Unmgd3_Yh_GLU %>%
    ...
    L2231.LN3_UnmgdCarbon[is.na(L2231.LN3_UnmgdCarbon)] <- 1

    # L2231.LN3_MgdCarbon_noncrop: Carbon content info, managed land in the third nest, non-crop (forest)
    L2231.LC_bm2_R_Mgd3_Yh_GLU %>%
      filter(year == max(MODEL_BASE_YEARS)) %>%
      left_join_error_no_match(select(GCAMLandLeaf_CdensityLT, Land_Type, LandLeaf), by = c("Land_Type" = "LandLeaf")) %>%
      rename(Cdensity_LT = Land_Type.y) %>%
    ...
    # L2231.LN3_NoEmissCarbon: Set the no-emiss-carbon-calc as the type of carbon to use in forest leaves.
    ...
    L2231.LN3_MgdCarbon_noncrop %>%
      ...
      bind_rows(L2231.LN3_NoEmissCarbon, .) %>%
    ...
    # L2231.LN3_NodeCarbon: Set the node-carbon-calc to drive the carbon calc between forest leaves
    L2231.LN3_NoEmissCarbon %>%
      select(-LandLeaf) %>%
      distinct() %>%
    ...
    bind_rows(L2231.LN3_HistUnmgdAllocation, L2231.LN3_UnmgdAllocation) %>%
      filter(!grepl("OtherArable", UnmanagedLandLeaf)) %>%
    ...
    # L2231.LN3_HistUnmgdAllocation_noprot: historical unmanaged land cover, no protect
    L2231.LN3_HistUnmgdAllocation_noprot <- create_noprot_unmgd(L2231.LN3_HistUnmgdAllocation, BYUnmgdAllocation, MINUnmgdAllocation)


    # L2231.LN3_UnmgdAllocation_noprot: unmanaged land cover, no protect
    L2231.LN3_UnmgdAllocation_noprot <- create_noprot_unmgd(L2231.LN3_UnmgdAllocation, BYUnmgdAllocation, MINUnmgdAllocation)
    ...
    # L2231.LN1_HistUnmgdAllocation_prot: historical unmanaged land cover, protected
    L2231.LN1_HistUnmgdAllocation_prot <- create_prot_unmgd(L2231.LN3_HistUnmgdAllocation, BYUnmgdAllocation, MINUnmgdAllocation)


    # L2231.LN1_UnmgdAllocation_prot: unmanaged land cover, protected
    L2231.LN1_UnmgdAllocation_prot <- create_prot_unmgd(L2231.LN3_UnmgdAllocation, BYUnmgdAllocation, MINUnmgdAllocation)


    # L2231.LN1_UnmgdCarbon_prot: unmanaged carbon info, protected
    L2231.LN3_UnmgdCarbon %>%
      filter(!grepl("OtherArable", UnmanagedLandLeaf)) %>%
      mutate(UnmanagedLandLeaf = paste0("Protected", UnmanagedLandLeaf),
             LandNode1 = UnmanagedLandLeaf,
    ...
    # L2231.LN1_Logit_prot: Logit for protected land in the first nest.
    L2231.LN1_UnmgdAllocation_prot %>%
    ...
    L1251.SoilTypeShare_R_Soil_LT_C_GLU <- L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      replace_GLU(map = basin_to_country_mapping)
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU <- L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      replace_GLU(map = basin_to_country_mapping)

    # Soil shares for land allocation on unmanaged lands
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      inner_join(A_LandLeaf_Unmgd3, by = c(Land_Type = "UnmanagedLandLeaf")) %>%
      rename(UnmanagedLandLeaf = Land_Type) %>%
      append_GLU("UnmanagedLandLeaf", "LandNode1", "LandNode2", "LandNode3") %>%
      select(region, LandNode1, LandNode2, LandNode3, UnmanagedLandLeaf, Soil_Type, share) ->
      L2231.LN3_Unmgd_SoilTypeShare

    # Soil shares for land allocation on managed lands
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      inner_join(A_LandLeaf3, by = c(Land_Type = "LandLeaf")) %>%
      rename(LandLeaf = Land_Type) %>%
      append_GLU("LandLeaf", "LandNode1", "LandNode2", "LandNode3") %>%
      select(region, LandNode1, LandNode2, LandNode3, LandLeaf, Soil_Type, share) ->
      L2231.LN3_Mgd_SoilTypeShare

    # Soil carbon densities on unmanaged lands...
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      inner_join(A_LandLeaf_Unmgd3, by = c(Land_Type = "UnmanagedLandLeaf")) %>%
      rename(UnmanagedLandLeaf = Land_Type) %>%
      append_GLU("UnmanagedLandLeaf", "LandNode1", "LandNode2", "LandNode3") %>%
      select(region, LandNode1, LandNode2, LandNode3, UnmanagedLandLeaf, soil_c) ->
      L2231.LN3_Unmgd_PeatSoilCarbon
    ...
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      ...
      rename(LandLeaf = Land_Type) %>%
      append_GLU("LandLeaf", "LandNode1", "LandNode2", "LandNode3") %>%
      select(region, LandNode1, LandNode2, LandNode3, LandLeaf, soil_c) ->
      L2231.LN3_Mgd_PeatSoilCarbon

    # Modified final tables
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      filter(Soil_Type == "Peat") %>%
      inner_join(select(A_LT_Mapping, LandNode1, LandNode2, LandNode3, Land_Type), by = "Land_Type") %>%
      append_GLU("LandNode1", "LandNode2", "LandNode3") %>%
      select(region, LandNode1, LandNode2, LandNode3, Soil_Type) %>%
      distinct() %>%
      inner_join(L2231.LN3_Logit, by = c("region", "LandNode1", "LandNode2", "LandNode3")) %>%
      mutate(LandNode1 = paste(LandNode1, Soil_Type, sep = aglu.PEAT_DELIMITER),
             LandNode2 = paste(LandNode2, Soil_Type, sep = aglu.PEAT_DELIMITER),
             LandNode3 = paste(LandNode3, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
    select(LEVEL2_DATA_NAMES[["LN3_Logit"]]) ->
      L2231.LN3_Logit_Peat
    L2231.LN3_Logit <- bind_rows(L2231.LN3_Logit, L2231.LN3_Logit_Peat)

    L2231.LN3_HistUnmgdAllocation <- apply_peat_landshares(L2231.LN3_HistUnmgdAllocation, L2231.LN3_Unmgd_SoilTypeShare)
    L2231.LN3_UnmgdAllocation <- apply_peat_landshares(L2231.LN3_UnmgdAllocation, L2231.LN3_Unmgd_SoilTypeShare)
    L2231.LN3_HistMgdAllocation_noncrop <- apply_peat_landshares(L2231.LN3_HistMgdAllocation_noncrop, L2231.LN3_Mgd_SoilTypeShare)
    L2231.LN3_MgdAllocation_noncrop <- apply_peat_landshares(L2231.LN3_MgdAllocation_noncrop, L2231.LN3_Mgd_SoilTypeShare)
    L2231.LN3_HistUnmgdAllocation_noprot <- apply_peat_landshares(L2231.LN3_HistUnmgdAllocation_noprot, L2231.LN3_Unmgd_SoilTypeShare)
    L2231.LN3_UnmgdAllocation_noprot <- apply_peat_landshares(L2231.LN3_UnmgdAllocation_noprot, L2231.LN3_Unmgd_SoilTypeShare)

    # Carbon density tables with peatland appended - use generic function
    L2231.LN3_UnmgdCarbon <- append_peat_carbon_info(L2231.LN3_UnmgdCarbon, L2231.LN3_Unmgd_PeatSoilCarbon)
    L2231.LN3_MgdCarbon_noncrop <- append_peat_carbon_info(L2231.LN3_MgdCarbon_noncrop, L2231.LN3_Mgd_PeatSoilCarbon)

    # No emissions node and carbon tables with peatland appended
    L2231.LN3_NoEmissCarbon_Peat <- L2231.LN3_NoEmissCarbon %>%
      semi_join(bind_rows(L2231.LN3_Unmgd_PeatSoilCarbon, L2231.LN3_Mgd_PeatSoilCarbon),
                by = c("region", "LandNode1", "LandNode2", "LandNode3"))

    L2231.LN3_NoEmissCarbon_Peat[c("LandNode1", "LandNode2", "LandNode3", "LandLeaf")] <-
      apply(L2231.LN3_NoEmissCarbon_Peat[c("LandNode1", "LandNode2", "LandNode3", "LandLeaf")], 2,
            function(x){x <- paste(x, "Peat", sep = aglu.PEAT_DELIMITER)})
    L2231.LN3_NoEmissCarbon <- bind_rows(L2231.LN3_NoEmissCarbon, L2231.LN3_NoEmissCarbon_Peat)

    L2231.LN3_NodeCarbon_Peat <- select(L2231.LN3_NoEmissCarbon_Peat, -LandLeaf) %>%
      distinct() %>%
      rename(node.carbon.calc = no.emiss.carbon.calc)

    L2231.LN3_NodeCarbon <- bind_rows(L2231.LN3_NodeCarbon, L2231.LN3_NodeCarbon_Peat)

    # Protected lands, allocation, disaggregated to peat and mineral soils
    L2231.LN3_Unmgd_SoilTypeShare_prot <- L2231.LN3_Unmgd_SoilTypeShare %>%
      select(-LandNode2, -LandNode3) %>%
      mutate(LandNode1 = paste0("Protected", UnmanagedLandLeaf),
             UnmanagedLandLeaf = LandNode1)
    L2231.LN3_Unmgd_PeatSoilCarbon_prot <- L2231.LN3_Unmgd_PeatSoilCarbon %>%
      select(-LandNode2, -LandNode3) %>%
      mutate(LandNode1 = paste0("Protected", UnmanagedLandLeaf),
             UnmanagedLandLeaf = LandNode1)
    L2231.LN1_HistUnmgdAllocation_prot <- apply_peat_landshares(L2231.LN1_HistUnmgdAllocation_prot, L2231.LN3_Unmgd_SoilTypeShare_prot)
    L2231.LN1_UnmgdAllocation_prot <- apply_peat_landshares(L2231.LN1_UnmgdAllocation_prot, L2231.LN3_Unmgd_SoilTypeShare_prot)

    # Protected lands, carbon density, with peatlands appended
    L2231.LN1_UnmgdCarbon_prot <- append_peat_carbon_info(L2231.LN1_UnmgdCarbon_prot, L2231.LN3_Unmgd_PeatSoilCarbon_prot)

    L2231.LN1_Logit_prot_Peat <- L2231.LN1_Logit_prot %>%
      inner_join(subset(L2231.LN3_Unmgd_SoilTypeShare_prot, Soil_Type == "Peat"),
                 by = c("region", "LandNode1")) %>%
      mutate(LandNode1 = paste(LandNode1, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
      select(names(L2231.LN1_Logit_prot))
    L2231.LN1_Logit_prot <- bind_rows(L2231.LN1_Logit_prot, L2231.LN1_Logit_prot_Peat)
    ...
    L2231.LN3_HistUnmgdAllocation_noprot %>%
      ...
      add_units("thou km2") %>%
      add_comments("Historical unmanaged land data from L2231.LN3_HistUnmgdAllocation is multiplied by ") %>%
      add_comments("specified fraction to give unprotected land allocation in the third nest. OtherArableLands omitted.") %>%
      add_legacy_name("L2231.LN3_HistUnmgdAllocation_noprot") %>%
      same_precursors_as(L2231.LN3_HistUnmgdAllocation) %>%
    ...
    L2231.LN3_UnmgdAllocation_noprot %>%
      ...
      add_units("thou km2") %>%
      add_comments("Unmanaged land data from L2231.LN3_UnmgdAllocation is multiplied by specified fraction to give ") %>%
      add_comments("unprotected land allocation in the third nest. OtherArableLands omitted.") %>%
      add_legacy_name("L2231.LN3_UnmgdAllocation_noprot") %>%
      same_precursors_as(L2231.LN3_UnmgdAllocation) %>%
      add_precursors("L126.ProtectFrac_R_LT_GLU")  ->
      L2231.LN3_UnmgdAllocation_noprot
    ...
    L2231.LN1_HistUnmgdAllocation_prot %>%
      ...
      add_legacy_name("L2231.LN1_HistUnmgdAllocation_prot") %>%
      same_precursors_as(L2231.LN3_HistUnmgdAllocation) %>%
      add_precursors("L126.ProtectFrac_R_LT_GLU") ->
      L2231.LN1_HistUnmgdAllocation_prot
    ...
    L2231.LN1_UnmgdAllocation_prot %>%
      ...
      add_legacy_name("L2231.LN1_UnmgdAllocation_prot") %>%
      same_precursors_as(L2231.LN3_UnmgdAllocation) %>%
      add_precursors("L126.ProtectFrac_R_LT_GLU") ->
      L2231.LN1_UnmgdAllocation_prot
    ...
    L2231.LN1_UnmgdCarbon_prot %>%
      ...
      add_legacy_name("L2231.LN1_UnmgdCarbon_prot") %>%
      same_precursors_as(L2231.LN3_UnmgdCarbon) ->
      L2231.LN1_UnmgdCarbon_prot
    ...
    L2231.LN1_Logit_prot %>%
      ...
      add_comments("are given externally defined constant logit information.") %>%
      add_legacy_name("L2231.LN1_Logit_prot") %>%
      same_precursors_as(L2231.LN1_UnmgdAllocation_prot) ->
      L2231.LN1_Logit_prot


    return_data(L2231.LN3_Logit, L2231.LN3_HistUnmgdAllocation, L2231.LN3_UnmgdAllocation, L2231.LN3_NoEmissCarbon,
                L2231.LN3_NodeCarbon, L2231.LN3_HistMgdAllocation_noncrop, L2231.LN3_MgdAllocation_noncrop,
                L2231.LN3_UnmgdCarbon, L2231.LN3_MgdCarbon_noncrop,
                # protected land outputs:
                L2231.LN3_HistUnmgdAllocation_noprot, L2231.LN3_UnmgdAllocation_noprot, L2231.LN1_HistUnmgdAllocation_prot,
                L2231.LN1_UnmgdAllocation_prot, L2231.LN1_UnmgdCarbon_prot, L2231.LN1_Logit_prot)
  } else {
