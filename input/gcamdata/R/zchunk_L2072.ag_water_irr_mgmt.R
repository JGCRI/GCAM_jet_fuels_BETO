...
#'   \code{L2072.AgCoef_IrrWaterWdraw_bio_mgmt}, \code{L2072.AgCoef_IrrWaterCons_bio_mgmt}. The corresponding file in the original data system was
...
module_aglu_L2072.ag_water_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1251.SoilTypeShare_R_Soil_LT_C_GLU <- get_data(all_data, "L1251.SoilTypeShare_R_Soil_LT_C_GLU")
    ...
    L2072.AgCoef_BphysWater_bio_mgmt %>%
      ...
      filter(!endsWith(AgSupplySubsector, "Peat")) %>%
    ...
    # Assign an exogenous floor on profit rates in order to prevent negative, zero, and very low profit rates
    # 12/12/2022 gpk - this is now done just by setting the tag cal-min-profit-rate at the AgSupplySector level, and the
    # C++ will calculate the necessary cost adjustments to keep profit rates positive
    L2052.AgCost_ag_irr_mgmt %>%
    ...
    L2072.UnAdjProfits %>%
      ...
      distinct() %>%
      mutate(cal.min.profit.rate = min(L2072.UnAdjProfits$Profit)) ->
      L2072.AgCalMinProfitRate
    ...
    L2072.AgCoef_IrrWaterCons_ag_mgmt <- append_peat_technologies(L2072.AgCoef_IrrWaterCons_ag_mgmt, L2072.ag_reg_peat)
    L2072.AgCoef_IrrWaterWdraw_ag_mgmt <- append_peat_technologies(L2072.AgCoef_IrrWaterWdraw_ag_mgmt, L2072.ag_reg_peat)
    L2072.AgCoef_IrrBphysWater_ag_mgmt <- append_peat_technologies(L2072.AgCoef_IrrBphysWater_ag_mgmt, L2072.ag_reg_peat)
    L2072.AgCoef_RfdBphysWater_ag_mgmt <- append_peat_technologies(L2072.AgCoef_RfdBphysWater_ag_mgmt, L2072.ag_reg_peat)
    L2072.AgCoef_BphysWater_bio_mgmt <- append_peat_technologies(L2072.AgCoef_BphysWater_bio_mgmt, L2072.ag_reg_peat)
    L2072.AgCoef_IrrWaterCons_bio_mgmt <- append_peat_technologies(L2072.AgCoef_IrrWaterCons_bio_mgmt, L2072.ag_reg_peat)
    L2072.AgCoef_IrrWaterWdraw_bio_mgmt <- append_peat_technologies(L2072.AgCoef_IrrWaterWdraw_bio_mgmt, L2072.ag_reg_peat)
    ...
    L2072.AgCalMinProfitRate %>%
      add_title("Minimum profit rate ($/kg) by region / crop type") %>%
      add_units("1975$/kg") %>%
      add_comments("This is the assumed minimum profit rate for each crop type, calculated from prices and non-land variable costs") %>%
