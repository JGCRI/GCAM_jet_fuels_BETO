...
#' append_peat_technologies
#'
#' Helper function to copy an existing table to corresponding technologies/subsectors on peat soil,
#' based on a separate table with the relevant subsectors on peat
#'
#' @param data Data to expand
#' @param peat_table table with peatland region, supplysector, subsector
#' @param join_vars "by" variables for the join between the peat table and the data table
#' @importFrom dplyr bind_rows inner_join intersect
#' @return full data table with peat info appended
append_peat_technologies <- function(data, peat_table, join_vars = intersect(names(data), names(peat_table))){

  assert_that(is_tibble(data))
  assert_that(is_tibble(peat_table))

  data_peat <- data %>%
    inner_join(peat_table,
               by = join_vars) %>%
    mutate(AgSupplySubsector = paste(AgSupplySubsector, Soil_Type, sep = aglu.PEAT_DELIMITER),
           AgProductionTechnology = paste(AgProductionTechnology, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
    select(names(data))
  data <- bind_rows(data, data_peat) %>%
    distinct()   # in case of duplicative appends
  return(data)
}


#' apply_peat_landshares
#'
#' Helper function to apply land share to a table of land allocations, appending the "Peat" string as appropriate,
#' multiplying land allocation quantities by peat (or mineral) shares
#'
#' @param data Data table to modify
#' @param peat_table table with relevant peatland region, land leaf, soil type, and share
#' @param join_vars "by" variables for the join between the peat table and the data table
#' @importFrom dplyr bind_rows inner_join intersect
#' @return full data table with peat info appended
apply_peat_landshares <- function(data, peat_table, join_vars = intersect(names(data), names(peat_table))){

  assert_that(is_tibble(data))
  assert_that(is_tibble(peat_table))

  data_full <- data %>%
    left_join(peat_table,
               by = join_vars)
  colnames_to_modify <- names(data)[grepl("LandNode", names(data)) | grepl("LandLeaf", names(data))]
  data_full[data_full$Soil_Type == "Peat", colnames_to_modify] <-
    apply(data_full[data_full$Soil_Type == "Peat", colnames_to_modify],
          2,
          function(x){x <- paste(x, "Peat", sep = aglu.PEAT_DELIMITER)})
  data_full <- mutate(data_full, allocation = round(allocation * share, aglu.DIGITS_LAND_USE))
  data_full <- data_full[names(data)]
  return(data_full)
}

#' apply_peat_emissions
#'
#' Helper function to apply land share to a table of burning emissions, appending the "Peat" string as appropriate,
#' multiplying emissions quantities by peat (or mineral) shares
#'
#' @param data Data table to modify
#' @param peat_table table with relevant peatland region, land leaf, soil type, and share
#' @param join_vars "by" variables for the join between the peat table and the data table
#' @importFrom dplyr bind_rows inner_join intersect
#' @return full data table with peat info appended
apply_peat_emissions <- function(data, peat_table, join_vars = intersect(names(data), names(peat_table))){
  ...
  data_full <- data %>%
    left_join(peat_table,
              by = join_vars)
  colnames_to_modify <- names(data)[grepl("AgSupplySubsector", names(data)) | grepl("LandTechnology", names(data))]
  data_full[data_full$Soil_Type == "Peat", colnames_to_modify] <-
    apply(data_full[data_full$Soil_Type == "Peat", colnames_to_modify],
          2,
          function(x){x <- paste(x, "Peat", sep = aglu.PEAT_DELIMITER)})
  data_full <- mutate(data_full, input.emissions = round(input.emissions * share, emissions.DIGITS_EMISSIONS))
  data_full <- data_full[names(data)]
  return(data_full)
}

#' append_peat_carbon_info
#'
#' Helper function to append carbon density information to peatlands to an existing table of carbon density information
#'
#' @param data Data table to modify
#' @param peat_table table with relevant peatland region, land leaf, soil type, and share
#' @param join_vars "by" variables for the join between the peat table and the data table
#' @importFrom dplyr bind_rows inner_join intersect
#' @return full data table with peat info appended
append_peat_carbon_info <- function(data, peat_table, join_vars = intersect(names(data), names(peat_table))){
  ...
  data_peat <- data %>%
    inner_join(peat_table,
               by = join_vars)
  colnames_to_modify <- names(data)[grepl("LandNode", names(data)) | grepl("LandLeaf", names(data))]
  data_peat[colnames_to_modify] <-
    apply(data_peat[colnames_to_modify],
          2,
          function(x){x <- paste(x, "Peat", sep = aglu.PEAT_DELIMITER)})
  data_peat$hist.soil.carbon.density <- data_peat$soil_c
  data_peat$soil.carbon.density <- data_peat$soil_c
  data_peat <- data_peat[names(data)]
  data <- bind_rows(data, data_peat) %>%
    distinct()   # in case of duplicative appends
  return(data)
}
