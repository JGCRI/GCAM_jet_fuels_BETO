...
module_aglu_L2252.land_input_5_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L181.LandShare_R_bio_GLU_irr <- get_data(all_data, "L181.LandShare_R_bio_GLU_irr", strip_attributes = TRUE)
    L181.LC_bm2_R_C_Yh_GLU_irr_level <- get_data(all_data, "L181.LC_bm2_R_C_Yh_GLU_irr_level", strip_attributes = TRUE)
    ...
    # convert_LN4_to_LN5
    # A function to carry LN4 information down to LN5, temporarily removing the "peat" string and re-appending it afterwards (where applicable)
    convert_LN4_to_LN5 <- function(data, names) {
      data %>%
        repeat_add_columns(tibble(level = c("lo", "hi"))) %>%
        mutate(Soil_Type = if_else(endsWith(LandLeaf, paste0(aglu.PEAT_DELIMITER, "Peat")), "Peat", "Mineral"),
               LandNode5 = sub(paste0(aglu.PEAT_DELIMITER, "Peat"), "", LandLeaf),
               LandLeaf = paste(LandNode5, level, sep = aglu.MGMT_DELIMITER),
               LandNode5 = if_else(Soil_Type == "Peat", paste(LandNode5, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode5),
               LandLeaf = if_else(Soil_Type == "Peat", paste(LandLeaf, Soil_Type, sep = aglu.PEAT_DELIMITER), LandLeaf)) ->
        data_new
    ...
    L2242.LN4_Logit %>%
      ...
      mutate(Soil_Type = if_else(endsWith(LandNode1, paste0(aglu.PEAT_DELIMITER, "Peat")), "Peat", "Mineral"),
             LandNode5 = paste(sub(paste0(aglu.PEAT_DELIMITER, "Peat"), "", LandNode4),
                               Irr_Rfd,
                               sep = aglu.IRR_DELIMITER),
             LandNode5 = if_else(Soil_Type == "Peat", paste(LandNode5, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode5),
    ...
    L181.LC_bm2_R_C_Yh_GLU_irr_level %>%
      ...
      select(region, GCAM_commodity, GCAM_subsector, GLU, Irr_Rfd, level, year, allocation, LandLeaf) ->
      L2252.LC_bm2_R_C_Yh_GLU_irr_mgmt

    # Expand this table to include peatlands disaggregation
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      replace_GLU(map = basin_to_country_mapping) %>%
      semi_join(L2252.LC_bm2_R_C_Yh_GLU_irr_mgmt, by = c("region", "GCAM_commodity", "GCAM_subsector", "GLU")) %>%
      select(region, GCAM_commodity, GCAM_subsector, GLU, Soil_Type, share) ->
      L2252.SoilTypeShare

    L2252.LC_bm2_R_C_Yh_GLU_irr_mgmt <- apply_peat_landshares(L2252.LC_bm2_R_C_Yh_GLU_irr_mgmt, L2252.SoilTypeShare) %>%
    ...
    # Adjust ALL_LAND_ALLOCATION so that historical land with zero production has zero land allocation
    ALL_LAND_ALLOCATION <- ALL_LAND_ALLOCATION %>%
      left_join(distinct(select(L2012.AgProduction_ag_irr_mgmt, region, year, AgProductionTechnology, calOutputValue)),
                               by = c("region", "year", "LandLeaf" = "AgProductionTechnology")) %>%
      mutate(allocation = if_else(calOutputValue == 0 & !is.na(calOutputValue), 0, allocation)) %>%
    ...
    # L2252.AgProduction_ag_irr_mgmt: historical ag production
    # adjust so that no non-zero production where we have zero land
    # this happens because of rounding, where we the land gets rounded to zero, but there is
    # some tiny amount of production remaining
    L2012.AgProduction_ag_irr_mgmt %>%
      left_join(distinct(select(bind_rows(L2252.LN5_HistMgdAllocation_crop, L2252.LN5_MgdAllocation_crop), region, year, LandLeaf, allocation)),
                by = c("region", "year", "AgProductionTechnology" = "LandLeaf")) %>%
      mutate(calOutputValue = if_else(allocation == 0 & !is.na(allocation), 0, calOutputValue)) %>%
      # filter(!(allocation == 0 & calOutputValue == 0)) %>% # Remove these 0 land/0 production crops
      select(-allocation) ->
      L2252.AgProduction_ag_irr_mgmt
    ...
    L171.ag_EcYield_kgm2_R_C_Y_GLU %>%
      ...
      mutate(LandNode5 = paste(GCAM_subsector, GLU, sep = aglu.CROP_GLU_DELIMITER),
             LandNode5 = paste(LandNode5, Irr_Rfd, sep = aglu.IRR_DELIMITER)) ->
      L2252.Cropland_Carbon

    # Just use the land allocation table as the base; inner_join to drop the peatlands (for now)
    L2252.LN5_MgdCarbon_crop <- L2252.LN5_MgdAllocation_crop %>%
      filter(year == max(year)) %>%
      inner_join(L2252.Cropland_Carbon, by = c("region", "LandNode5")) %>%
      select(LEVEL2_DATA_NAMES[["LN5_MgdCarbon"]])

    # Peatland disaggregation - append modified carbon contents
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      replace_GLU(map = basin_to_country_mapping) %>%
      semi_join(L2252.Cropland_Carbon, by = c("region", "GCAM_commodity", "GLU")) %>%
      mutate(LandNode4 = paste(GCAM_subsector, GLU, sep = aglu.CROP_GLU_DELIMITER)) %>%
      select(region, LandNode4, soil_c) ->
      L2252.PeatCarbonContent

    L2252.LN5_MgdCarbon_crop <- append_peat_carbon_info(L2252.LN5_MgdCarbon_crop, L2252.PeatCarbonContent)

    # bio carbon contents - remove peat from the starting data, and append it to the end using the generic function
    L2012.AgYield_bio_ref %>%
      filter(year == max(MODEL_BASE_YEARS),
             !endsWith(AgProductionTechnology, paste0(aglu.PEAT_DELIMITER, "Peat"))) %>%
      select(region, AgProductionTechnology, yield) %>%
      separate(AgProductionTechnology, c("GCAM_commodity", "GLU", "Irr_Rfd", "level")) %>%
      #mutate(GCAM_commodity = paste(GCAM_commodity, biomass_type, sep = "_")) %>%
    ...
    # Peatland disaggregation - bio
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      replace_GLU(map = basin_to_country_mapping) %>%
      inner_join(filter(GCAMLandLeaf_CdensityLT, LandLeaf %in% c("biomassGrass", "biomassTree")),
                 by = "Land_Type") %>%
      mutate(LandNode4 = paste(LandLeaf, GLU, sep = aglu.LT_GLU_DELIMITER)) %>%
      select(region, LandNode4, soil_c) ->
      L2252.PeatCarbonContent_bio

    L2252.LN5_MgdCarbon_bio <- append_peat_carbon_info(L2252.LN5_MgdCarbon_bio, L2252.PeatCarbonContent_bio)
    ...
    L2252.LN5_MgdAllocation_bio %>%
      ...
      mutate(tmp = sub(paste0(aglu.PEAT_DELIMITER, "Peat"), "", LandLeaf)) %>%
    ...
    L2252.LN5_HistMgdAllocation_crop %>%
      ...
      add_comments("Adjusted to zero out land allocation where crop production is zero") %>%
    ...
    L2252.AgProduction_ag_irr_mgmt %>%
      ...
      add_comments("Adjusted to zero out production where land allocation is zero") %>%
    ...
    return_data(L2252.LN5_Logit, L2252.LN5_HistMgdAllocation_crop, L2252.LN5_MgdAllocation_crop,
                L2252.LN5_HistMgdAllocation_bio, L2252.LN5_MgdAllocation_bio, L2252.LN5_MgdCarbon_crop,
                L2252.LN5_MgdCarbon_bio, L2252.LN5_LeafGhostShare, L2252.LN5_NodeGhostShare, L2252.AgProduction_ag_irr_mgmt)
  } else {
