...
module_aglu_LA101.ag_FAO_R_C_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    GLU <- default_share_GLU <- HA_share_GLU <- prod_share_GLU <-
      ...
      item <- iso <- production <- harvested.area <- output <- feedcake <-
      oil <-  NULL # silence package check.
    ...
    L100.FAO_ag_Prod2_t <- get_data(all_data, "L100.FAO_ag_Prod2_t")
    ...
    # Prod2: calculate production of processed primary agricultural products
    L100.FAO_ag_Prod2_t %>%
      select(-countries, -country.codes, -item.codes, -element, -element.codes) %>%                               # Remove unnecessary columns
      left_join_error_no_match(iso_GCAM_regID, by = "iso") %>%                                                   # Map in ISO codes
      left_join(select(FAO_ag_items_cal_SUA, item, GCAM_commodity), by = "item") %>%                                                           # Map in GCAM commodities
      filter(!is.na(GCAM_commodity)) %>%                                                                          # Remove commodities not included in GCAM
      mutate(value = value * CONV_TON_MEGATON) %>%                                                               # Convert from tons to Mt
      group_by(GCAM_region_ID, GCAM_commodity, year) %>%                                                          # Group by region, commodity, year
    ...
    # Crushing: for the feedcake and oil commodities, calculate the inputs to the crushing process
    # For the oilseeds (sunflower, rapeseed, soybean), the crop input to the crushing process is calculated
    # as the sum of the mass of the oil, the feedcakes, and the (assumed) water losses.
    # For oil palm fruit, where the crop is combusted, the crop input is calculated as the oil divided by the
    # assumed crop oil content
    L101.ag_Crushing_Mt_R_C_Y <- filter(L101.ag_Prod2_Mt_R_C_Y, grepl(" oil| feedcake", GCAM_commodity)) %>%
      separate(GCAM_commodity, c("GCAM_commodity", "output"), sep = " ") %>%
      spread(key = output, value = value) %>%
      mutate(value = if_else(GCAM_commodity == "PalmFruit",
                             oil / aglu.PALMFRUIT_OILCONTENT,
                             (feedcake + oil) * (1 + aglu.CRUSHING_LOSS_FACTOR))) %>%
    ...
    L101.ag_Prod2_Mt_R_C_Y %>%
      add_title("Agricultural production (from SUA database) by GCAM region, commodity, and year") %>%
      add_units("Mt/yr") %>%
      add_comments("FAO SUA data downscaled to GLU then aggregated by GCAM region and commodity") %>%
      add_comments("Data was also converted from tons to Mt") %>%
      add_comments("This dataset includes secondary crop commodities (e.g. oil, feedcakes)") %>%
      add_precursors("L100.FAO_ag_Prod2_t", "aglu/FAO/FAO_ag_items_cal_SUA", "common/iso_GCAM_regID") ->
      L101.ag_Prod2_Mt_R_C_Y

    L101.ag_Crushing_Mt_R_C_Y %>%
      add_title("Agricultural inputs to crushing (oil and meal production) by GCAM region, commodity, and year") %>%
      add_units("Mt/yr") %>%
      add_comments("FAO SUA data downscaled to GLU then aggregated by GCAM region and commodity") %>%
      add_comments("Oil and meal commodities added, and multiplied by an exogenous loss factor") %>%
      same_precursors_as(L101.ag_Prod2_Mt_R_C_Y) ->
      L101.ag_Crushing_Mt_R_C_Y
    ...
    return_data(L101.ag_Food_Mt_R_C_Y, L101.ag_Food_Pcal_R_C_Y, L101.ag_kcalg_R_C_Y, L101.ag_HA_bm2_R_C_Y_GLU,
                L101.ag_HA_bm2_R_C_Y, L101.ag_Prod_Mt_R_C_Y_GLU, L101.ag_Prod_Mt_R_C_Y, L101.ag_Prod2_Mt_R_C_Y,
                L101.ag_Crushing_Mt_R_C_Y, L101.ag_Yield_kgm2_R_C_Y_GLU)
  } else {
