...
module_emissions_L2111.ag_nonco2_IRR <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2111.AGRBio <- L211.AGRBio %>%
      repeat_add_columns(Irr_Rfd) %>%
      mutate(AgProductionTechnology = paste(AgProductionTechnology, Irr_Rfd, sep = aglu.IRR_DELIMITER)) %>%
      select(-Irr_Rfd)

    L2111.AWB_BCOC_EmissCoeff <- L211.AWB_BCOC_EmissCoeff %>%
      repeat_add_columns(Irr_Rfd) %>%
      mutate(AgProductionTechnology = paste(AgProductionTechnology, Irr_Rfd, sep = aglu.IRR_DELIMITER)) %>%
      select(-Irr_Rfd)

    L2111.nonghg_max_reduction <- L211.nonghg_max_reduction %>%
      repeat_add_columns(Irr_Rfd) %>%
      mutate(AgProductionTechnology = paste(AgProductionTechnology, Irr_Rfd, sep = aglu.IRR_DELIMITER)) %>%
      select(-Irr_Rfd)

    L2111.nonghg_steepness <- L211.nonghg_steepness %>%
      repeat_add_columns(Irr_Rfd) %>%
      mutate(AgProductionTechnology = paste(AgProductionTechnology, Irr_Rfd, sep = aglu.IRR_DELIMITER)) %>%
      select(-Irr_Rfd)
    ...
    L2111.AGREmissions %>%
      ...
      add_units("Tg") %>%
      add_comments("AGR emissions, not downscaled to peat vs mineral soils") %>%
      add_comments("All emissions at this stage are assigned to mineral soils, and will be re-assigned subsequently") %>%
