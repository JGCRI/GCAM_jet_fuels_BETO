...
module_aglu_L2012.ag_For_Past_bio_input_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Disaggregate crop production to peat versus mineral soils, in regions where this distinction pertains
    # Mineral soils are not named specifically, so the names stay the same, but the calibration quantities need to be adjusted

    # Start by getting the correct AgSupplySector, AgSupplySubsector combinations, since there are now C4 crops
    AgSupplyMatch <- L2012.AgProduction_ag_irr_mgmt %>%
      select(region, AgSupplySector, AgSupplySubsector) %>%
      distinct() %>%
      separate(col = AgSupplySubsector, into = c("delete", "GLU_name"), remove = FALSE) %>%
      select(-delete)
    ...
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      ...
      left_join_error_no_match(select(basin_to_country_mapping, GLU_code, GLU_name), by = c("GLU" = "GLU_code")) %>%
      mutate(AgSupplySector = GCAM_commodity,
             AgSupplySubsector = if_else(is.na(GCAM_subsector),
                                         paste(AgSupplySector, GLU_name, sep = aglu.CROP_GLU_DELIMITER),
                                         paste(GCAM_subsector, GLU_name, sep = aglu.CROP_GLU_DELIMITER))) %>%
      select(region, AgSupplySector, AgSupplySubsector, Soil_Type, share) ->
      L2012.SoilTypeShare_R_Subs

    # For each of these tables, just paste the "Peat" designation to the end of the existing subsector and/or technology
    # names, where applicable. In this way, all strings will have the same order of ID information
    # L2012.AgProduction_ag_irr_mgmt
    L2012.AgProduction_ag_irr_mgmt %>%
      left_join(L2012.SoilTypeShare_R_Subs,
                by = c("region", "AgSupplySector", "AgSupplySubsector")) %>%
      mutate(AgSupplySubsector = if_else(Soil_Type == "Mineral",
                                         AgSupplySubsector,
                                         paste(AgSupplySubsector, Soil_Type, sep = aglu.PEAT_DELIMITER)),
             AgProductionTechnology = if_else(Soil_Type == "Mineral",
                                              AgProductionTechnology,
                                              paste(AgProductionTechnology, Soil_Type, sep = aglu.PEAT_DELIMITER)),
             calOutputValue = round(calOutputValue * share, aglu.DIGITS_CALOUTPUT),
             subs.share.weight = if_else(share > 0, subs.share.weight, 0),
             tech.share.weight = if_else(share > 0, tech.share.weight, 0)) %>%
      select(LEVEL2_DATA_NAMES[["AgProduction"]]) ->
      L2012.AgProduction_ag_irr_mgmt
    ...
    # L2012.AgHAtoCL_irr_mgmt - use the generic function as this is just being copied
    L2012.AgHAtoCL_irr_mgmt <- append_peat_technologies(L2012.AgHAtoCL_irr_mgmt,
                                                        filter(L2012.SoilTypeShare_R_Subs, Soil_Type == "Peat"))

    # Bioenergy related data. First filter the peat soil data to the appropriate combinations
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      filter(Land_Type %in% c("Forest", "HarvCropLand"),
             Soil_Type == "Peat") %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      left_join_error_no_match(select(basin_to_country_mapping, GLU_code, GLU_name), by = c("GLU" = "GLU_code")) %>%
      mutate(AgSupplySector = "biomass",
             AgSupplySubsector = if_else(Land_Type == "HarvCropLand",
                                         paste("biomassGrass", GLU_name, sep = aglu.CROP_GLU_DELIMITER),
                                         paste("biomassTree", GLU_name, sep = aglu.CROP_GLU_DELIMITER))) %>%
      select(region, AgSupplySector, AgSupplySubsector, Soil_Type) %>%
      distinct() ->
      L2012.bio_reg_peat

    # L2012.AgSupplySubsector
    L2012.AgSupplySubsector_Peat_ag <- L2012.AgSupplySubsector %>%
      inner_join(filter(L2012.SoilTypeShare_R_Subs, Soil_Type == "Peat"),
                 by = c("region", "AgSupplySector", "AgSupplySubsector")) %>%
      mutate(AgSupplySubsector = paste(AgSupplySubsector, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
      select(LEVEL2_DATA_NAMES[["AgSupplySubsector"]])
    L2012.AgSupplySubsector_Peat_bio <- L2012.AgSupplySubsector %>%
      inner_join(L2012.bio_reg_peat, by = c("region", "AgSupplySector", "AgSupplySubsector")) %>%
      mutate(AgSupplySubsector = paste(AgSupplySubsector, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
      select(LEVEL2_DATA_NAMES[["AgSupplySubsector"]])

    L2012.AgSupplySubsector <- bind_rows(L2012.AgSupplySubsector, L2012.AgSupplySubsector_Peat_ag, L2012.AgSupplySubsector_Peat_bio)

    # L2012.AgYield_bio_ref
    L2012.AgYield_bio_ref <- append_peat_technologies(L2012.AgYield_bio_ref, L2012.bio_reg_peat)
   ...
   L2012.AgSupplySubsector %>%
    ...
    return_data(L2012.AgSupplySector, L2012.AgSupplySubsector, L2012.AgProduction_ag_irr_mgmt, L2012.AgProduction_For,
                L2012.AgProduction_Past, L2012.AgHAtoCL_irr_mgmt, L2012.AgYield_bio_ref, L2012.AgTechYr_Past)
  } else {
