...
module_aglu_L2052.ag_prodchange_cost_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L162.bio_YieldRate_R_Y_GLU_irr %>%
      ...
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      left_join_error_no_match(basin_to_country_mapping[c("GLU_code", "GLU_name")], by = c("GLU" = "GLU_code")) %>%
      repeat_add_columns(tibble(biocrop_type = c("grass", "tree"))) %>%
      repeat_add_columns(tibble(mgmt_level = c("hi", "lo"))) %>%
      mutate(AgSupplySector = GCAM_commodity,
             AgSupplySubsector = paste(AgSupplySector, biocrop_type, GLU_name, sep = aglu.CROP_GLU_DELIMITER),
             AgProductionTechnology = paste(AgSupplySubsector, Irr_Rfd, mgmt_level, sep = aglu.IRR_DELIMITER)) %>%
      select(region, AgSupplySector, AgSupplySubsector, AgProductionTechnology, year, AgProdChange = value) ->
    ...
    # Table of ag production subsectors on peatland
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      ...
      mutate(AgSupplySector = GCAM_commodity,
             AgSupplySubsector = paste(if_else(is.na(GCAM_subsector), AgSupplySector, GCAM_subsector), GLU_name, sep = aglu.CROP_GLU_DELIMITER)) %>%
      select(region, AgSupplySector, AgSupplySubsector, Soil_Type) ->
      L2052.ag_reg_peat

    # table of bioenergy production subsectors on peatland
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      ...
      mutate(AgSupplySector = "biomass",
             AgSupplySubsector = if_else(Land_Type == "HarvCropLand",
                                         paste("biomass_grass", GLU_name, sep = aglu.CROP_GLU_DELIMITER),
                                         paste("biomass_tree", GLU_name, sep = aglu.CROP_GLU_DELIMITER))) %>%
      select(region, AgSupplySector, AgSupplySubsector, Soil_Type) %>%
      distinct() ->
      L2052.bio_reg_peat

    # Use the generic function for all of these tables, as the same values for mineral soils are copied for peat soils
    L2052.AgCost_ag_irr_mgmt <- append_peat_technologies(L2052.AgCost_ag_irr_mgmt, L2052.ag_reg_peat)
    L2052.AgCost_bio_irr_mgmt <- append_peat_technologies(L2052.AgCost_bio_irr_mgmt, L2052.bio_reg_peat)
    L2052.AgCost_For <- append_peat_technologies(L2052.AgCost_For, L2052.ag_reg_peat)
    L2052.AgProdChange_ag_irr_ref <- append_peat_technologies(L2052.AgProdChange_ag_irr_ref, L2052.ag_reg_peat)
    L2052.AgProdChange_bio_irr_ref <- append_peat_technologies(L2052.AgProdChange_bio_irr_ref, L2052.bio_reg_peat)
    L2052.AgProdChange_irr_high <- append_peat_technologies(L2052.AgProdChange_irr_high, L2052.ag_reg_peat)
    L2052.AgProdChange_irr_low <- append_peat_technologies(L2052.AgProdChange_irr_low, L2052.ag_reg_peat)
    L2052.AgProdChange_irr_ssp4 <- append_peat_technologies(L2052.AgProdChange_irr_ssp4, L2052.ag_reg_peat)
    ...
    L2052.AgCost_bio_irr_mgmt %>%
      ...
      add_precursors("L1251.SoilTypeShare_R_Soil_LT_C_GLU", "L2012.AgYield_bio_ref") ->
      L2052.AgCost_bio_irr_mgmt
    ...
    L2052.AgProdChange_bio_irr_ref %>%
      ...
      add_precursors("L1251.SoilTypeShare_R_Soil_LT_C_GLU",
                     "L162.bio_YieldRate_R_Y_GLU_irr",
                     "L2012.AgYield_bio_ref") ->
      L2052.AgProdChange_bio_irr_ref
