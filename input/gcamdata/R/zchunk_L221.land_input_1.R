...
#'
#' Produce L221.LN0_Logit, L221.LN0_Land, L221.LN0_SoilTimeScale, L221.LN1_SoilTimeScale_Peat, L221.LN1_ValueLogit,
...
#' \item{"L221.LN1_SoilTimeScale_Peat Soil time scale by region.
#' External soil time scale assumptions for each land node on peat soil."}
#' #TODO check whether this description makes sense ^
...
module_aglu_L221.land_input_1 <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L131.LV_USD75_m2_R_GLU <- get_data(all_data, "L131.LV_USD75_m2_R_GLU", strip_attributes = TRUE)
    ...
    A_soil_time_scale_R %>%
      ...
      mutate(LandAllocatorRoot = "root") ->
      L221.LN0_SoilTimeScale
    #see peat expansion section below for L221.LN1_PeatSoilTimeScale
    ...
    # Expand tables to peatlands, where appropriate
    # First, make the tables with relevant peatland information.
    # Available lands, for setting logit exponents and unmanaged land value...
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      filter(Soil_Type == "Peat") %>%
      left_join_error_no_match(distinct(select(A_LT_Mapping, LandNode1, Land_Type)),
                               by = "Land_Type") %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      left_join_error_no_match(select(basin_to_country_mapping, GLU_code, GLU_name), by = c("GLU" = "GLU_code")) %>%
      mutate(LandNode1 = paste(LandNode1, GLU_name, sep = aglu.LT_GLU_DELIMITER)) %>%
      select(region, LandNode1, Soil_Type) %>%
      distinct() ->
      L221.LN1_R_GLU

    # ...land allocation shares...
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      filter(Land_Type %in% A_LandLeaf_Unmgd1$UnmanagedLandLeaf) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      left_join_error_no_match(select(basin_to_country_mapping, GLU_code, GLU_name), by = c("GLU" = "GLU_code")) %>%
      mutate(UnmanagedLandLeaf = paste(Land_Type, GLU_name, sep = aglu.LT_GLU_DELIMITER)) %>%
      select(region, UnmanagedLandLeaf, Soil_Type, share) ->
      L221.LN1_SoilTypeShare_R_GLU

    #... and soil carbon density
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      filter(Land_Type %in% A_LandLeaf_Unmgd1$UnmanagedLandLeaf) %>%
      left_join_error_no_match(GCAM_region_names, by = "GCAM_region_ID") %>%
      left_join_error_no_match(select(basin_to_country_mapping, GLU_code, GLU_name), by = c("GLU" = "GLU_code")) %>%
      mutate(UnmanagedLandLeaf = paste(Land_Type, GLU_name, sep = aglu.LT_GLU_DELIMITER)) %>%
      select(region, UnmanagedLandLeaf, soil_c) ->
      L221.LN1_PeatSoilCarbon_R_GLU

    # Use generic functions to perform the expansions
    L221.LN1_ValueLogit_Peat <- L221.LN1_ValueLogit %>%
      inner_join(L221.LN1_R_GLU, by = c("region", "LandNode1")) %>%
      mutate(LandNode1 = paste(LandNode1, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
      select(LEVEL2_DATA_NAMES[["LN1_ValueLogit"]])
    L221.LN1_ValueLogit <- bind_rows(L221.LN1_ValueLogit, L221.LN1_ValueLogit_Peat)
    L221.LN1_HistUnmgdAllocation <- apply_peat_landshares(L221.LN1_HistUnmgdAllocation, L221.LN1_SoilTypeShare_R_GLU)
    L221.LN1_UnmgdAllocation <- apply_peat_landshares(L221.LN1_UnmgdAllocation, L221.LN1_SoilTypeShare_R_GLU)
    L221.LN1_UnmgdCarbon <- append_peat_carbon_info(L221.LN1_UnmgdCarbon, L221.LN1_PeatSoilCarbon_R_GLU)
    L221.LN1_SoilTimeScale_Peat <- L221.LN1_R_GLU %>%
      mutate(LandNode1 = paste(LandNode1, Soil_Type, sep = aglu.PEAT_DELIMITER)) %>%
      mutate(LandAllocatorRoot = "root") %>%
      mutate(nodeSoilTimeScale = 300) %>% #hardcoding all peat timescales for now
      #in the future: either create a new input CSV (e.g. A_LandNode_PeatSoil_time_scale or relate to L221.LN0_SoilTimeScale somehow)
      select(LEVEL2_DATA_NAMES[["LN1_SoilTimeScale_Peat"]])
    ...
    L221.LN0_SoilTimeScale %>%
      ...
      L221.LN0_SoilTimeScale
    L221.LN1_SoilTimeScale_Peat %>%
      add_title("Peat soil time scale by land use region with peat", overwrite = T) %>% #TODO: check description
      add_units("rate") %>%
      add_comments("#TODO write these") %>%
      add_legacy_name("L221.LN1_SoilTimeScale_Peat", overwrite = T) -> #%>%
      #add_precursors(#TODO confirm that there aren't any) ->
      L221.LN1_SoilTimeScale_Peat
    L221.LN1_ValueLogit %>%
    ...
    L221.LN1_UnmgdAllocation %>%
      ...
      same_precursors_as(L221.LN1_HistUnmgdAllocation) ->
      L221.LN1_UnmgdAllocation
    L221.LN1_UnmgdCarbon %>%
    ...
    return_data(L221.LN0_Logit, L221.LN0_Land, L221.LN0_SoilTimeScale, L221.LN1_ValueLogit,
                L221.LN1_HistUnmgdAllocation, L221.LN1_UnmgdAllocation, L221.LN1_UnmgdCarbon, L221.LN1_SoilTimeScale_Peat)
  } else {
