...
module_aglu_LA108.ag_Feed_R_C_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    iso <- item <- year <- value <- GCAM_commodity <- GCAM_region_ID <- feed <-
      ...
      feedcakes <- ddgs_feedcakes <- value_prod <- value_feed <-
      feed_statdiff_scaler <- NULL # silence package check.
    ...
    L100.FAO_ag_Prod2_t <- get_data(all_data, "L100.FAO_ag_Prod2_t", strip_attributes = T)
    L101.ag_Prod_Mt_R_C_Y <- get_data(all_data, "L101.ag_Prod_Mt_R_C_Y", strip_attributes = T)
    L101.ag_Prod2_Mt_R_C_Y <- get_data(all_data, "L101.ag_Prod2_Mt_R_C_Y", strip_attributes = T)
    L1061.ALL_Mt_R_DDGS_Y <- get_data(all_data, "L1061.ALL_Mt_R_DDGS_Y", strip_attributes = T)
    L107.an_Feed_Mt_R_C_Sys_Fd_Y <- get_data(all_data, "L107.an_Feed_Mt_R_C_Sys_Fd_Y", strip_attributes = T)
    L122.FeedOut_Mt_R_C_Yh <- get_data(all_data, "L122.FeedOut_Mt_R_C_Yh", strip_attributes = T)

    # 11/20/2019 revision - up front adjustment to FAO feed data. There are two commodities, "Oilseed Cakes, Other" and
    # Palmkernel Cake", that have more global feed demands than production in some years. As these commodities don't really
    # have any other uses (e.g., seed, waste, industrial processing), and almost all of the demands are feed related,
    # this situation is unique to these commodities and requires an up-front adjustment to the demands in several years

    # The scalers, where applied, are multiplied by 0.9995 so that aggregated rounding errors don't lead to negative values

    L108.oilcake_statdiff_scaler <- filter(L100.FAO_ag_Feed_t, item %in% c("Oilseed Cakes, Other", "Palmkernel Cake")) %>%
      left_join(L100.FAO_ag_Prod2_t, by = c("iso", "country.codes", "item", "year"), suffix = c("_feed", "_prod")) %>%
      replace_na(list(value_prod = 0)) %>%
      group_by(item, year) %>%
      summarise(value_prod = sum(value_prod), value_feed = sum(value_feed)) %>%
      ungroup() %>%
      mutate(feed_statdiff_scaler = if_else(value_feed > value_prod, value_prod * 0.9995 / value_feed, 1)) %>%
      select(item, year, feed_statdiff_scaler)

    L100.FAO_ag_Feed_t <- left_join(L100.FAO_ag_Feed_t, L108.oilcake_statdiff_scaler,
                                    by = c("item", "year")) %>%
      mutate(value = if_else(is.na(feed_statdiff_scaler), value, value * feed_statdiff_scaler)) %>%
      select(-feed_statdiff_scaler)

    # 10/01/2019 revision - include DDGS. This is somewhat complicated. FAOSTAT includes oilcrop feedcakes
    # (e.g., soybean, rapeseed) that are co-produced with oils, irrespective of whether the oil is later used to produce
    # biofuels. So, the co-produced feedcakes are already in the data, handled upstream of the biomassOil sector.
    # However, DDGS from corn ethanol production are excluded from reported Corn -> Feed in FAOSTAT. For this
    # reason, the methods below treat DDGS specifically.

    L108.DDGS <- select(L122.FeedOut_Mt_R_C_Yh, -GCAM_commodity) %>%
      rename(GCAM_commodity = fractional.secondary.output) %>%
      filter(grepl("DDGS", GCAM_commodity)) %>%
      ...
      complete(nesting(GCAM_commodity, year), GCAM_region_ID = sort(unique(iso_GCAM_regID$GCAM_region_ID)),
               fill = list(value = 0)) %>%
      # 10/09/2019 - js and gpk - we need to replace the feed output data, production, with the actual domestic
      # consumption of DDGS in each region. (This is to properly account for DDGS trade)
      left_join(select(L1061.ALL_Mt_R_DDGS_Y, GCAM_region_ID, GCAM_commodity, year, Feed_Mt),
                by = c("GCAM_region_ID", "GCAM_commodity", "year")) %>%
      mutate(value = if_else(is.na(Feed_Mt), value, Feed_Mt)) %>%
    ...
    # 10/23/2019 - for disaggregating feedcakes, which can't be scaled, need to pull the feedcakes
    # out of the table prior to computing shares
    L108.feedcakes <- subset(ag_Feed_Mt_R_Cnf_Y, grepl("feedcake", GCAM_commodity))
    ag_Feed_Mt_R_Cnf_Y <- subset(ag_Feed_Mt_R_Cnf_Y, !grepl("feedcake", GCAM_commodity)) %>%
    ...
    # 4/12/2019 revision, cont'd: because DDGS and oil crop feedcakes are coming from calibrated technologies in
    # quantities that can not be scaled, and because they aren't traded and are assigned 100% to feed, the values in
    # L108.DDGS_feedcakes_total need to be subtracted from this IMAGE-based animal feed total, with the remainder
    # apportioned to the other (i.e., non-DDGS and feedcakes) feedcrops. The steps below deduct the DDGS and feedcakes
    # totals from the animal feed quantity computed above.
    L108.DDGS_feedcakes <- bind_rows(L108.DDGS, L108.feedcakes)
    L108.DDGS_feedcakes_total <- L108.DDGS_feedcakes %>%
      group_by(GCAM_region_ID, year) %>%
      summarise(ddgs_feedcakes = sum(value)) %>%
      ungroup()
    ...
    an_Feed_Mt_R_C_Y %>%
      ...
      left_join_error_no_match(L108.DDGS_feedcakes_total,
    ...
    ag_Feed_Mt_R_C_Y %>%
      ...
      add_precursors("common/iso_GCAM_regID", "aglu/FAO/FAO_ag_items_cal_SUA", "L100.FAO_ag_Feed_t", "L100.FAO_ag_Prod2_t", "aglu/A_recent_feed_modifications",
                     "L101.ag_Prod_Mt_R_C_Y", "L101.ag_Prod2_Mt_R_C_Y", "L1061.ALL_Mt_R_DDGS_Y", "L107.an_Feed_Mt_R_C_Sys_Fd_Y", "L122.FeedOut_Mt_R_C_Yh") ->
