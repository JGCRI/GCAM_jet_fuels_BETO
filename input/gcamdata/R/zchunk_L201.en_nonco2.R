...
module_emissions_L201.en_nonco2 <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    bind_rows(
      ...
      get_data(all_data, UCD_tech_map_name) %>%
        select(supplysector, subsector = tranSubsector, fuel, technology = tranTechnology, rev_input) %>%
        rename(minicam.energy.input = rev_input)) %>%
