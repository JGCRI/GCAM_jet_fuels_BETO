...
module_energy_L221.en_supply <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
   ...
   # GPK 10/10/2021 - commenting this out for now as these technologies should still be allowed in the global tech database.
    L221.StubTech_en %>%
    ...
    L221.GlobalTechCoef_en %>%
       mutate(coefficient= if_else(minicam.energy.input =="crude oil",
                                   if_else(year <= MODEL_FINAL_BASE_YEAR,1,coefficient),coefficient)) -> L221.GlobalTechCoef_en
    ...
    #update 04/10 JS: L221.ddgs_regions should include all GCAM regions as they may import DDGS
    #L221.globaltech_secout_R %>%
    A_regions %>%
     distinct(region)%>%
      filter(region!=aglu.NO_AGLU_REGIONS)-> L221.ddgs_regions

    #10/10 JS: Indonesia and Southeast Asia do not have Oil meal feedackes and they are not traded in this version of the model.
    feedcake.techs<-c('Soybean','OilCrop','Rapeseed')
    #regions<-select(A_regions,region)

    L221.oil.meal_regions<- L221.biofuel_types_region %>%
      filter(supplysector=='regional biomassOil'
             ,technology %in% feedcake.techs) %>%
         distinct(region)
    #L221.oil.meal_regions<-anti_join(regions, L221.oil.meal_regions, by="region")
    ...
    # 2/12/2021 gpk - also de-rate the "feed price" used for setting the price point at which 100% of the sec out ratio is met,
    # in order to ensure that this fraction stays constant in the scenarios
    FractSecOut_FeedPrice_Derate <- 0.8
    L221.StubTechFractSecOut_en %>%
      select(-output.ratio) %>%
      mutate(P0 = 0) %>%
      left_join(L221.ag_FeedPrice_R_Yf, by = "region") %>%
      mutate(P1 = round(feed_price * FractSecOut_FeedPrice_Derate, digits = energy.DIGITS_COST)) %>%
    ...
    # Update JS 10-25-2019: Decrease the price at which the outputs become fractional (divided by two)
    L221.StubTechFractProd_en<- L221.StubTechFractProd_en %>%
      mutate(price=if_else(supplysector=='regional corn for ethanol'&
                             subsector=='regional corn for ethanol'&
                             stub.technology=="regional corn for ethanol"&
    ...
    #10/10 JS: the ddgs and oil meal feedcakes need to be treated differently in L221.DepRsrc_en, as some regions will import DDG
    # but they cannot import Oil Meal feedcakes
    A21.rsrc_info %>%
      filter(resource=='DDGS') %>%
    ...
    #10/10 JS:Same for the price calculation
    A21.rsrc_info %>%
      filter(resource=='DDGS') %>%
    ...
    L221.StubTechFractSecOut_en %>%
      ...
      add_precursors("aglu/A_agRegionalTechnology", "energy/A21.globaltech_secout", "common/GCAM_region_names", "L122.in_Mt_R_C_Yh", "L121.BiomassOilRatios_kgGJ_R_C") ->
      L221.StubTechFractSecOut_en
