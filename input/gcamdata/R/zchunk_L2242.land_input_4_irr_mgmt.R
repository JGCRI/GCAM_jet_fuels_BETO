...
module_aglu_L2242.land_input_4_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Here, the input data tables have peatlands already disaggregated; just keep track of them (rather than appending post-hoc)
    L2012.AgProduction_ag_irr_mgmt %>%
      bind_rows(L2012.AgYield_bio_ref) %>%
      mutate(Soil_Type = if_else(endsWith(AgSupplySubsector, paste0(aglu.PEAT_DELIMITER, "Peat")),
                                 "Peat",
                                 "Mineral"),
             AgSupplySubsector = sub(paste0(aglu.PEAT_DELIMITER, "Peat"), "", AgSupplySubsector)) %>%
      distinct(region, AgSupplySector, AgSupplySubsector, Soil_Type) %>%
      # LandLeaf is everything before underscore
      mutate(LandLeaf = sub("_.*$", "", AgSupplySubsector)) %>%
      left_join_error_no_match(A_LandLeaf3, by="LandLeaf") %>%
      ...
      separate(AgSupplySubsector, c("LandNode4", "GLU")) %>%
      ...
      append_GLU("LandNode1", "LandNode2", "LandNode3", "LandNode4") %>%
      mutate(LandNode1 = if_else(Soil_Type == "Peat", paste(LandNode1, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode1),
             LandNode2 = if_else(Soil_Type == "Peat", paste(LandNode2, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode2),
             LandNode3 = if_else(Soil_Type == "Peat", paste(LandNode3, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode3),
             LandNode4 = if_else(Soil_Type == "Peat", paste(LandNode4, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode4)) %>%
    ...
    a<-L2012.AgYield_bio_ref %>%
      mutate(Soil_Type = if_else(endsWith(AgSupplySubsector, paste0(aglu.PEAT_DELIMITER, "Peat")),
                                 "Peat",
                                 "Mineral"),
             AgSupplySubsector = sub(paste0(aglu.PEAT_DELIMITER, "Peat"), "", AgSupplySubsector)) %>%
      distinct(region, AgSupplySubsector, Soil_Type) %>%
      mutate(GCAM_commodity = if_else(grepl("^biomassGrass", AgSupplySubsector), "biomassGrass", "biomassTree"),
             GLU = if_else(grepl("^biomassGrass", AgSupplySubsector), gsub("biomassGrass_", "", AgSupplySubsector),
                           gsub("biomassTree_", "", AgSupplySubsector))) %>%
      left_join_error_no_match(A_LT_Mapping, by = "GCAM_commodity") %>%
      mutate(LandAllocatorRoot = "root") %>%
      append_GLU("LandNode1", "LandNode2", "LandNode3", "LandLeaf") %>%
      ...
      mutate(ghost.unnormalized.share = approx_fun(year, ghost.share),
             LandNode1 = if_else(Soil_Type == "Peat", paste(LandNode1, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode1),
             LandNode2 = if_else(Soil_Type == "Peat", paste(LandNode2, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode2),
             LandNode3 = if_else(Soil_Type == "Peat", paste(LandNode3, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode3),
             LandNode4 = if_else(Soil_Type == "Peat", paste(LandNode4, Soil_Type, sep = aglu.PEAT_DELIMITER), LandNode4)) %>%
