...
module_aglu_L2062.ag_Fert_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Peatland disaggregation - expand this table to all peatlands here
    # This is somewhat different from other cases of peatland expansion, in that there are other "level2" data tables
    # that have already been expanded that will be joined with this one. So for consistency, expand here.
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
    ...
    L2062.AgCoef_Fert_ag_irr_mgmt <- append_peat_technologies(L2062.AgCoef_Fert_ag_irr_mgmt, L2062.ag_reg_peat)

    # Calculate fertilizer coefficients for grassy bioenergy crops
    A_Fodderbio_chars %>%
      filter(GCAM_commodity == "biomassGrass") %>%
      mutate(coefficient = round((aglu.BIO_GRASS_FERT_IO_GNM2 * CONV_G_KG / aglu.BIO_GRASS_YIELD_KGCM2    # Convert from application per unit area to per unit carbon
    ...
    A_Fodderbio_chars %>%
      ...
      mutate(coefficient = round((aglu.BIO_TREE_FERT_IO_GNM2 * CONV_G_KG / aglu.BIO_TREE_YIELD_KGCM2    # Convert from application per unit area to per unit carbon
                            * aglu.CCONTENT_CELLULOSE * (1 - WaterContent))                         # Convert from carbon to wet biomass
             / (aglu.BIO_ENERGY_CONTENT_GJT * CONV_KG_T), aglu.DIGITS_CALOUTPUT)) ->                         # Convert from biomass to energy
      bio_tree_coef
