...
module_aglu_L222.land_input_2 <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      inner_join(A_LandLeaf_Unmgd2, by = c(Land_Type = "UnmanagedLandLeaf")) %>%
      rename(UnmanagedLandLeaf = Land_Type) %>%
      append_GLU("UnmanagedLandLeaf", "LandNode1", "LandNode2") %>%
      select(region, LandNode1, LandNode2, UnmanagedLandLeaf, Soil_Type, share) ->
      L222.LN2_Unmgd_SoilTypeShare

    # Soil shares for land allocation on managed lands
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      inner_join(A_LandLeaf2, by = c(Land_Type = "LandLeaf")) %>%
      rename(LandLeaf = Land_Type) %>%
      append_GLU("LandLeaf", "LandNode1", "LandNode2") %>%
      select(region, LandNode1, LandNode2, LandLeaf, Soil_Type, share) ->
      L222.LN2_Mgd_SoilTypeShare
    ...
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      ...
      rename(UnmanagedLandLeaf = Land_Type) %>%
      append_GLU("UnmanagedLandLeaf", "LandNode1", "LandNode2") %>%
      select(region, LandNode1, LandNode2, UnmanagedLandLeaf, soil_c) ->
      L222.LN2_Unmgd_PeatSoilCarbon
    ...
    L1251.PeatCarbonContent_kgm2_R_LT_C_GLU %>%
      ...
      rename(LandLeaf = Land_Type) %>%
      append_GLU("LandLeaf", "LandNode1", "LandNode2") %>%
      select(region, LandNode1, LandNode2, LandLeaf, soil_c) ->
      L222.LN2_Mgd_PeatSoilCarbon

    #Modified final tables
    # Logit exponents apply to all GLUs that peatland and append to the existing logit exponent table
    # L222.LN2_Logit: Logit exponent of the second land nest (down two nests from the top-level nest)
    L1251.SoilTypeShare_R_Soil_LT_C_GLU %>%
      filter(Soil_Type == "Peat") %>%
      inner_join(select(A_LT_Mapping, LandNode1, LandNode2, Land_Type), by = "Land_Type") %>%
      append_GLU("LandNode1", "LandNode2") %>%
      select(region, LandNode1, LandNode2, Soil_Type) %>%
      distinct() %>%
      inner_join(L222.LN2_Logit, by = c("region", "LandNode1", "LandNode2")) %>%
      ...
      select(LEVEL2_DATA_NAMES[["LN2_Logit"]]) ->
      L222.LN2_Logit_Peat
    L222.LN2_Logit <- bind_rows(L222.LN2_Logit, L222.LN2_Logit_Peat)

    L222.LN1_Logit_prot_Peat <- L222.LN1_Logit_prot %>%
      mutate(UnmanagedLandLeaf = substr(LandNode1, nchar("Protected") + 1, nchar(LandNode1))) %>%
      semi_join(L222.LN2_Unmgd_PeatSoilCarbon, by = c("region", "UnmanagedLandLeaf")) %>%
      mutate(LandNode1 = paste(LandNode1, "Peat", sep = aglu.PEAT_DELIMITER)) %>%
      select(names(L222.LN1_Logit_prot))
    L222.LN1_Logit_prot <- bind_rows(L222.LN1_Logit_prot, L222.LN1_Logit_prot_Peat)

    # Land allocation tables - use generic function
    L222.LN2_HistUnmgdAllocation <- apply_peat_landshares(L222.LN2_HistUnmgdAllocation, L222.LN2_Unmgd_SoilTypeShare)
    L222.LN2_UnmgdAllocation <- apply_peat_landshares(L222.LN2_UnmgdAllocation, L222.LN2_Unmgd_SoilTypeShare)
    L222.LN2_HistMgdAllocation <- apply_peat_landshares(L222.LN2_HistMgdAllocation, L222.LN2_Mgd_SoilTypeShare)
    L222.LN2_MgdAllocation <- apply_peat_landshares(L222.LN2_MgdAllocation, L222.LN2_Mgd_SoilTypeShare)
    L222.LN2_HistUnmgdAllocation_noprot <- apply_peat_landshares(L222.LN2_HistUnmgdAllocation_noprot, L222.LN2_Unmgd_SoilTypeShare)
    L222.LN2_UnmgdAllocation_noprot <- apply_peat_landshares(L222.LN2_UnmgdAllocation_noprot, L222.LN2_Unmgd_SoilTypeShare)

    # Carbon density tables - use generic function
    L222.LN2_UnmgdCarbon <- append_peat_carbon_info(L222.LN2_UnmgdCarbon, L222.LN2_Unmgd_PeatSoilCarbon)
    L222.LN2_MgdCarbon <- append_peat_carbon_info(L222.LN2_MgdCarbon, L222.LN2_Mgd_PeatSoilCarbon)

    # Protected lands, allocation
    L222.LN2_Unmgd_SoilTypeShare_prot <- L222.LN2_Unmgd_SoilTypeShare %>%
      select(-LandNode2) %>%
      mutate(LandNode1 = paste0("Protected", UnmanagedLandLeaf),
             UnmanagedLandLeaf = LandNode1)
    L222.LN2_Unmgd_PeatSoilCarbon_prot <- L222.LN2_Unmgd_PeatSoilCarbon %>%
      select(-LandNode2) %>%
      mutate(LandNode1 = paste0("Protected", UnmanagedLandLeaf),
             UnmanagedLandLeaf = LandNode1)
    L222.LN1_HistUnmgdAllocation_prot <- apply_peat_landshares(L222.LN1_HistUnmgdAllocation_prot, L222.LN2_Unmgd_SoilTypeShare_prot)
    L222.LN1_UnmgdAllocation_prot <- apply_peat_landshares(L222.LN1_UnmgdAllocation_prot, L222.LN2_Unmgd_SoilTypeShare_prot)

    # Protected lands, carbon density
    L222.LN1_UnmgdCarbon_prot <- append_peat_carbon_info(L222.LN1_UnmgdCarbon_prot, L222.LN2_Unmgd_PeatSoilCarbon_prot)
    ...
    L222.LN2_UnmgdAllocation %>%
      ...
      same_precursors_as(L222.LN2_HistUnmgdAllocation) ->
      L222.LN2_UnmgdAllocation
    ...
    L222.LN2_MgdAllocation %>%
      ...
      same_precursors_as(L222.LN2_HistMgdAllocation) ->
      L222.LN2_MgdAllocation
    ...
    L222.LN2_MgdCarbon %>%
      ...
      same_precursors_as(L222.LN2_UnmgdCarbon) ->
      L222.LN2_MgdCarbon

    L222.LN2_Unmgd_PeatSoilCarbon %>%
      add_title("Carbon content for unmanaged land (LT_GLU) in second nest on peat soils by region.") %>%
      add_units("Varies") %>%
      add_comments("Carbon content info for unmanaged land (LT_GLU) on peat soils (only used in L212.unmgd_nonco2 to identify peat GLUS)") %>%
      add_legacy_name("L222.LN2_Unmgd_PeatSoilCarbon") %>%
      same_precursors_as(L222.LN2_UnmgdCarbon) ->
      L222.LN2_Unmgd_PeatSoilCarbon
    ...
    L222.LN2_UnmgdAllocation_noprot %>%
      ...
      same_precursors_as(L222.LN2_HistUnmgdAllocation_noprot) ->
      L222.LN2_UnmgdAllocation_noprot
    ...
    L222.LN1_UnmgdAllocation_prot %>%
      ...
      same_precursors_as(L222.LN1_HistUnmgdAllocation_prot) ->
      L222.LN1_UnmgdAllocation_prot
    ...
    return_data(L222.LN2_Logit, L222.LN2_HistUnmgdAllocation, L222.LN2_UnmgdAllocation,
                L222.LN2_HistMgdAllocation, L222.LN2_MgdAllocation, L222.LN2_UnmgdCarbon, L222.LN2_MgdCarbon,
                L222.LN2_HistUnmgdAllocation_noprot, L222.LN2_UnmgdAllocation_noprot, L222.LN1_Logit_prot,
                L222.LN1_HistUnmgdAllocation_prot, L222.LN1_UnmgdAllocation_prot, L222.LN1_UnmgdCarbon_prot, L222.LN2_Unmgd_PeatSoilCarbon)
  } else {
