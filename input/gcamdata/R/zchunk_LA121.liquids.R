...
#'   \code{L121.in_EJ_R_supply_crude_Yh}, \code{L121.in_EJ_R_TPES_unoil_Yh}, \code{L121.share_R_TPES_biofuel_tech}. The
...
module_energy_LA121.liquids <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1262.refinedOil_GrossTrade_EJ_R_C_Y <- get_data(all_data, "L1262.refinedOil_GrossTrade_EJ_R_C_Y", strip_attributes = T)
    ...
    if(is.null(L100.IEA_en_bal_ctry_hist)) {
      ...
      L121.in_EJ_R_supply_crude_Yh <- extract_prebuilt_data("L121.in_EJ_R_supply_crude_Yh")
      ...
      L111.Prod_EJ_R_F_Yh <- get_data(all_data, "L111.Prod_EJ_R_F_Yh", strip_attributes = T)
      ...
      # Due to regionalizing refined liquids, crude oil regional consumption must be altered by the amount of refined liquids traded
      # by each region
      ...
      L121.share_R_TPES_biofuel_tech <- filter(L100.IEA_en_bal_ctry_hist,
        ...
        left_join(bio_feed_mapping %>%
                        select(GCAM_commodity=biomassOil_tech, new_tech=technology)
                   ,by = "GCAM_commodity") %>%
        mutate(technology = if_else(!is.na(new_tech),new_tech,technology)) %>%
        select(-new_tech)
      ...
      L121.in_EJ_R_supply_crude_Yh %>%
        add_title("Crude oil crude oil demand by GCAM region / historical year adjusted for regional refined liquids trade") %>%
        add_units("EJ") %>%
