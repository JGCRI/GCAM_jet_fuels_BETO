...
module_aglu_L2042.resbio_input_irr_mgmt <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Res bio parameters are copied from mineral soils to peat soils, so just use the generic function for expansion
    ## - this produces an XML file that is too big. So, just keep resbio out of the peat technologies for now.
    #L204.AgResBio_For <- append_peat_technologies(L204.AgResBio_For, L2042.Peat_resbio)
    #L204.AgResBioCurve_For <- append_peat_technologies(L204.AgResBioCurve_For, L2042.Peat_resbio)
    #L2042.AgResBio_ag_irr_mgmt <- append_peat_technologies(L2042.AgResBio_ag_irr_mgmt, L2042.Peat_resbio)
    #L2042.AgResBioCurve_ag_irr_mgmt <- append_peat_technologies(L2042.AgResBioCurve_ag_irr_mgmt, L2042.Peat_resbio)
    ...
    L204.AgResBioCurve_For %>%
      ...
      same_precursors_as(L2042.AgResBio_For) %>%
      add_precursors("aglu/A_resbio_curves") ->
      L2042.AgResBioCurve_For
    ...
    L2042.AgResBioCurve_ag_irr_mgmt %>%
      ...
      same_precursors_as(L2042.AgResBio_ag_irr_mgmt) ->
      L2042.AgResBioCurve_ag_irr_mgmt
