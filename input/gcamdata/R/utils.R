...
find_chunks <- function(pattern = "^module_[a-zA-Z\\.]*_.*$", include_disabled = FALSE) {
  ...
  ls(name = parent.env(environment()), pattern = pattern) %>%
    ...
    # These gcamusa chunks are needed to run the emissions module, so they can't be disabled
    mutate(disabled = if_else(name %in% c("module_gcamusa_L270.nonghg_nei_to_gcam",
                                          "module_gcamusa_L170.nonghg_ceds_scaling_USA",
                                          "module_gcamusa_L169.nonghg_NEI_scaling_USA") & !("emissions" %in% c(DISABLED_MODULES)),
                              FALSE,
                              disabled)) %>%
