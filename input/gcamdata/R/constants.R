...
aglu.TRADED_CROPS           <- c("Corn", "FiberCrop", "Fruits", "Legumes", "MiscCrop", "NutsSeeds", "OilCrop", "OilCrop feedcake", "OilCrop oil",
                                 "OtherGrain", "PalmFruit", "PalmFruit oil", "PalmFruit feedcake", "Rapeseed",
                                 "Rapeseed feedcake", "Rapeseed oil", "Rice", "RootTuber", "Soybean", "Soybean feedcake",
                                 "Soybean oil", "SugarCrop", "Vegetables", "Wheat")
...
aglu.TRADED_AG_AN           <- c(aglu.TRADED_CROPS, aglu.TRADED_MEATS, aglu.TRADED_FORESTS)
aglu.TRADED_AG_AN_withDDG   <- c(aglu.TRADED_CROPS, aglu.TRADED_MEATS, aglu.TRADED_FORESTS, "DDGS")
...
aglu.CRUSHING_LOSS_FACTOR <- 0.021 # Unitless portion of ag commodities that are lost (evaporation, other losses) in crushing crops into oil and meal. Source: Table 2 in https://www.sciencedirect.com/science/article/pii/S0960852417321648?via%3Dihub
aglu.PALMFRUIT_OILCONTENT <- 0.24 # Value provided by OTAQ (JGCRIOT-178) is 0.2125, but this results in negative other uses in IDN and SEA. 0.24 is a better match for their data.
...
# Margin for feedcake output ratio from veg oil production. When price of feedcakes drops this portion below the base-year level, sec out coef starts to decrease
aglu.FEEDCAKE_PRICE_MARGIN <- 0.2
...
aglu.PEAT_DELIMITER       <- "_"  # delimiter between existing subsector/technology names and the peat soil name
...
energy.AGEN_PRICE <- 18 # average agricultural energy use price in 2015

# Ratio of agricultural energy assumed to go to crop production
# Based on USDA Farm Production expenditures - https://downloads.usda.library.cornell.edu/usda-esmis/files/qz20ss48r/zp38wg225/v979v575v/FarmProdEx-08-04-2016.pdf
energy.CROP_ENERGY_RATIO <- 0.65
