...
module_energy_batch_resource_energy_inputs_xml <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2101.ResTechCoef_fos <- get_data(all_data, "L2101.ResTechCoef_fos")
    L2101.ResTechCoefAdj_fos <- get_data(all_data, "L2101.ResTechCoefAdj_fos")

    # #=======#=======#=======#=======#=======#=======#=========

    # Produce outputs
    create_xml("resource_energy_inputs.xml") %>%
      add_xml_data(L2101.ResTechCoef_fos, "ResReserveTechCoef") %>%
      add_xml_data(L2101.ResTechCoefAdj_fos, "ResReserveTechCoefAdj") %>%
      add_precursors("L2101.ResTechCoef_fos",
                     "L2101.ResTechCoefAdj_fos") ->
