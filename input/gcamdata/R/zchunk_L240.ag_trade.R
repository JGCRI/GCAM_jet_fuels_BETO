...
module_aglu_L240.ag_trade <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L240.Production_tra <- filter(A_agTradedTechnology_R_Y, year %in% MODEL_BASE_YEARS) %>%
      #JS 10/10 change "no match" for trying
      left_join(L240.GrossExports_Mt_R_C_Y,
    ...
    L240.Production_reg_imp <- A_agRegionalTechnology_R_Y %>%
      ...
      #JS 10/10 change "no match" for trying
      left_join(L240.GrossImports_Mt_R_C_Y,
    ...
    # 9/25/2019 - regions with zero DDGS production in all time periods (i.e., those without corn ethanol technologies) should not
    # have "domestic" subsectors of DDGS. These technologies will be pulling from a local DDGS production market that does not exist.
    # no DDG regions: c('Brazil','Central America and Caribbean','Indonesia','South America_Northern','Southeast Asia','Taiwan','Colombia' )


    #JS 10/10 add the joining variables to the anti.join functions in order to avoid the warning messages
     no_ddgs_regions <- unique(A_regions$region[A_regions$ethanol != "corn ethanol"])

    L240.SubsectorAll_reg.anti <- filter(L240.SubsectorAll_reg,
                   region %in% no_ddgs_regions,
                   subsector=='domestic DDGS')
    L240.SubsectorAll_reg<-anti_join(L240.SubsectorAll_reg, L240.SubsectorAll_reg.anti, by = c("region", "supplysector", "subsector", "logit.year.fillout", "logit.exponent", "year.fillout", "share.weight", "apply.to", "from.year", "to.year", "interpolation.function", "logit.type"))


    L240.TechShrwt_reg.anti <- filter(L240.TechShrwt_reg,
                                      region %in% no_ddgs_regions,
                                      subsector=='domestic DDGS')
    L240.TechShrwt_reg<-anti_join(L240.TechShrwt_reg, L240.TechShrwt_reg.anti,by = c("region", "supplysector", "subsector", "technology", "year", "share.weight"))


    L240.TechCoef_reg.anti <- filter(L240.TechCoef_reg,
                                     region %in% no_ddgs_regions,
                                     subsector=='domestic DDGS')
    L240.TechCoef_reg<-anti_join(L240.TechCoef_reg, L240.TechCoef_reg.anti,by = c("region", "supplysector", "subsector", "technology", "year", "minicam.energy.input", "coefficient", "market.name"))


    L240.Production_reg_dom.anti <- filter(L240.Production_reg_dom,
                                           region %in% no_ddgs_regions,
                                           subsector=='domestic DDGS')
    L240.Production_reg_dom<-anti_join(L240.Production_reg_dom, L240.Production_reg_dom.anti,by = c("region", "supplysector", "subsector", "technology", "year", "calOutputValue", "share.weight.year", "subs.share.weight", "tech.share.weight"))
