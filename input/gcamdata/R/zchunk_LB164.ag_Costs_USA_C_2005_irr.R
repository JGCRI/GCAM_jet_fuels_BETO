...
module_aglu_LB164.ag_Costs_USA_C_2005_irr <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # 11/24/2019 revision - if any of the commodities (e.g. oil palm) aren't grown in the USA,
    # fill out default values (just assume 100% rainfed)
    L164.ag_irrHA_frac_USA_C <- complete(L164.ag_irrHA_frac_USA_C,
                                         nesting(GCAM_region_ID), GCAM_commodity = unique(L133.ag_Cost_75USDkg_C$GCAM_commodity)) %>%
