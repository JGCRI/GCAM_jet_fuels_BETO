...
module_aglu_LB109.ag_an_ALL_R_C_Y <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    year <- value <- GCAM_region_ID <- GCAM_commodity <- . <- flow <- Feed_Mt <-
      Prod_Mt <- NetExp_Mt <- Supply_Mt <- Food_Mt <- Biofuels_Mt <- Crushing_Mt <-
    ...
    L101.ag_Prod2_Mt_R_C_Y <- get_data(all_data, "L101.ag_Prod2_Mt_R_C_Y")
    L101.ag_Crushing_Mt_R_C_Y <- get_data(all_data, "L101.ag_Crushing_Mt_R_C_Y")
    ...
    L1061.ALL_Mt_R_DDGS_Y <- get_data(all_data, "L1061.ALL_Mt_R_DDGS_Y")
    ...
    ag_Flow_cols <- c("Prod_Mt", "NetExp_Mt", "Supply_Mt", "Food_Mt", "Feed_Mt", "Biofuels_Mt", "Crushing_Mt", "OtherUses_Mt")

    # Feed-only commodities that aren't in ag production data
    Secondary_feeds_for_balance <- unique(L108.ag_Feed_Mt_R_C_Y$GCAM_commodity[!L108.ag_Feed_Mt_R_C_Y$GCAM_commodity %in% L101.ag_Prod_Mt_R_C_Y$GCAM_commodity])
    # need to remove DDGS as this was processed in LA1061
    Secondary_feeds_for_balance <- Secondary_feeds_for_balance[Secondary_feeds_for_balance != "DDGS"]
    ...
    # 10/23/2019 - need to combine the production of "feed commodities" (oils and feedcakes) with the primary (crop) commodities
    # in the production table
    L101.ag_Prod_Mt_R_C_Y <- bind_rows(L101.ag_Prod_Mt_R_C_Y,
                                       filter(L101.ag_Prod2_Mt_R_C_Y, GCAM_commodity %in% Feed_commodities))
    ...
    L106.ag_NetExp_Mt_R_C_Y %>%
      ...
      bind_rows(mutate(L101.ag_Crushing_Mt_R_C_Y, flow = "Crushing_Mt")) %>%
...
#           mutate(Prod_Mt = if_else(GCAM_commodity %in% Secondary_feeds_for_balance, Feed_Mt, Prod_Mt),
      mutate(Prod_Mt = if_else(GCAM_commodity %in% Feed_commodities & !GCAM_commodity %in% L101.ag_Prod2_Mt_R_C_Y$GCAM_commodity,
                               Feed_Mt, Prod_Mt),
             # Calculate the domestic supply
             Supply_Mt = Prod_Mt - NetExp_Mt,
             # Calculate other uses
             OtherUses_Mt = Supply_Mt - Food_Mt - Feed_Mt - Biofuels_Mt - Crushing_Mt) -> L109.ag_ALL_Mt_R_C_Y
    ...
    if(any(L109.ag_ALL_Mt_R_C_Y$OtherUses_Mt < 0)) {
      L109.ag_ALL_Mt_R_C_Y %>%
        ...
        ungroup -> L109.ag_ALL_Mt_glbl_C_Y
      ...
      L109.ag_ALL_Mt_R_C_Y %>%
        ...
        mutate(NetExpAdjFrac = if_else(GlobalOtherUses_Mt == 0, 0, OtherUses_Mt_adj / GlobalOtherUses_Mt),
               ...
               Supply_Mt = Prod_Mt - NetExp_Mt,
               OtherUses_Mt = Supply_Mt - Food_Mt - Biofuels_Mt - Feed_Mt - Crushing_Mt) %>%
    ...
    # Final step (gpk and js, 10/09/2019): bind in the DDGS mass balance
    L109.ag_ALL_Mt_R_C_Y <- bind_rows(L109.ag_ALL_Mt_R_C_Y,
                                      L1061.ALL_Mt_R_DDGS_Y[names(L1061.ALL_Mt_R_DDGS_Y) %in% names(L109.ag_ALL_Mt_R_C_Y)]) %>%
      arrange(GCAM_region_ID, GCAM_commodity, year)
