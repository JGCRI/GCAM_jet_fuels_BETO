...
module_aglu_LB1091.ag_GrossTrade <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    FAO_ag_items_TRADE <- get_data(all_data, "aglu/FAO/FAO_ag_items_TRADE") %>%
      mutate(GCAM_commodity=if_else(GCAM_commodity=="Root_Tuber","RootTuber",GCAM_commodity))
    L109.an_ALL_Mt_R_C_Y <- get_data(all_data, "L109.an_ALL_Mt_R_C_Y", strip_attributes = TRUE)
    FAO_BilateralTrade <- get_data(all_data, "FAO_BilateralTrade")
    L1061.ALL_Mt_R_DDGS_Y <- get_data(all_data, "L1061.ALL_Mt_R_DDGS_Y", strip_attributes = TRUE)
    L109.ag_ALL_Mt_R_C_Y <- get_data(all_data, "L109.ag_ALL_Mt_R_C_Y", strip_attributes = TRUE) %>%
      # filter out DDGS here, since were adding it in later with 1061
    ...
    L1091.XregTrade_Mt_R_C <- L1091.BiTrade_t_ctry_item_full %>%
      ...
      complete(GCAM_region_ID = unique(GCAM_region_ID),
               GCAM_commodity = c(aglu.TRADED_CROPS, aglu.TRADED_MEATS),
               var = unique(var)) %>%
    ...
    # All commodities in the ag mass balances need to be written out
    L1091.GrossTrade_Mt_R_C_fhy <- complete(L1091.GrossTrade_Mt_R_C_fhy,
                                            nesting(GCAM_region_ID, year),
                                            GCAM_commodity = sort(unique(L109.ag_ALL_Mt_R_C_Y$GCAM_commodity)),
                                            fill = list(GrossExp_Mt = 0, GrossImp_Mt = 0))
    ...
    L1091.GrossTrade_Mt_R_C_Y <- filter(L109.ag_an_ALL_Mt_R_C_Y, year != aglu.TRADE_FINAL_BASE_YEAR) %>%
      ...
      bind_rows(L1061.ALL_Mt_R_DDGS_Y[names(L1061.ALL_Mt_R_DDGS_Y) %in% names(L1091.GrossTrade_Mt_R_C_fhy)])



    # Produce outputs
    L1091.GrossTrade_Mt_R_C_Y %>%
      add_title("Gross trade of primary agricultural products, by region / commodity / year.", overwrite = T) %>%
      add_units("Mt") %>%
