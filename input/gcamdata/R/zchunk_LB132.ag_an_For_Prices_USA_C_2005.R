...
module_aglu_LB132.ag_an_For_Prices_USA_C_2005 <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    country.codes <- item.codes <- element <- element.codes <- year <- price <-
      ...
      `country codes` <- `item codes` <- `element codes` <- deflator <- . <-
      rev_item <- NULL    # silence package check.
    ...
    # 11/19/2019 gpk modification - due to asymmetrical crop name changes in the various FAOSTAT databases, the same
    # item codes correspond to different item names in the prices and production databases. In order to not interfere
    # with the "extra" commodities made in the first steps below (which don't necessarily have item codes), the
    # commodity names in the price data are changed here
    ag_item_names <- distinct(select(FAO_ag_Prod_t_PRODSTAT, item, item.codes))
    an_item_names <- distinct(select(FAO_USA_an_Prod_t_PRODSTAT, item, item.codes))
    fao_item_names <- bind_rows(ag_item_names, an_item_names) %>%
      rename(rev_item = item)

    FAO_USA_ag_an_P_USDt_PRICESTAT <- FAO_USA_ag_an_P_USDt_PRICESTAT %>%
      left_join(fao_item_names, by = "item.codes") %>%
      mutate(item = if_else(is.na(rev_item), item, rev_item)) %>%
      select(-rev_item)
