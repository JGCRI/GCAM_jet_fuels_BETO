...
module_energy_LA1011.en_bal_adj <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    # Adjustment for energy inputs to resource production: the way IO coefficients that change over time
    # for a given vintage are handled, a region/resource can't start out with a zero coef and later
    # become a non-zero number. So, here we set a minimum IO coef and re-compute the energy consumption
    # by resource production in each region.

    # First, compile the resource production, adding oil and gas in order to keep the same categories as the energy consumption
    L1011.out_EJ_R_rsrcFos_Yh <- filter(L1011.en_bal_EJ_R_Si_Fi_Yh,
                                        sector == "out_resources",
                                        fuel %in% c("refined liquids", "gas", "coal")) %>%
      mutate(fuel = if_else(fuel == "coal", "coal", "oilgas")) %>%
      group_by(GCAM_region_ID, sector, fuel, year) %>%
      summarise(value = sum(value)) %>%
      ungroup() %>%
      rename(energy_input_sector = fuel)

    # Next, subset the energy inputs to resource production, re-naming columns for joining
    L1011.in_EJ_R_rsrcFos_Yh <- filter(L1011.en_bal_EJ_R_Si_Fi_Yh,
                                        sector %in% c("in_industry_oilgas", "in_industry_coal"),
                                        fuel %in% c("electricity", "gas", "refined liquids")) %>%
      mutate(sector = sub("in_industry_", "", sector)) %>%
      rename(energy_input_sector = sector,
             energy_input_fuel = fuel,
             energy_input_value = value)

    # Join these two tables together, and aggregate all years in order to compute the
    # average coefficient over all historical years. The minimum IO coef is then assumed
    # equal to 1% of the average over all years, for each region, resource, and input fuel type.
    L1011.min_max_IOcoefs <- L1011.out_EJ_R_rsrcFos_Yh %>%
      left_join(L1011.in_EJ_R_rsrcFos_Yh, by = c("GCAM_region_ID", "energy_input_sector", "year")) %>%
      group_by(GCAM_region_ID, sector, energy_input_sector, energy_input_fuel) %>%
      summarise(value = sum(value),
                energy_input_value = sum(energy_input_value)) %>%
      ungroup() %>%
      group_by(sector, energy_input_sector, energy_input_fuel) %>%
      mutate(min_energy_IOcoef = energy_input_value * 1e-2 / value,
             max_energy_IOcoef = sum(energy_input_value) * 3 / sum(value)) %>%
      ungroup() %>%
      select(GCAM_region_ID, energy_input_sector, energy_input_fuel, min_energy_IOcoef, max_energy_IOcoef)

    # Recalculate the energy_input_values by region, energy_input_sector, energy_input_fuel, and year
    # Revised value: where original estimate was zero, replace with resource production * minimum IO coef
    # Only rows where the energy_input_value changes are kept
    L1011.in_EJ_R_rsrcFos_Yh_revised <- L1011.in_EJ_R_rsrcFos_Yh %>%
      left_join_error_no_match(L1011.out_EJ_R_rsrcFos_Yh, by = c("GCAM_region_ID", "energy_input_sector", "year")) %>%
      left_join_error_no_match(L1011.min_max_IOcoefs,
                               by = c("GCAM_region_ID", "energy_input_sector", "energy_input_fuel")) %>%
      filter(energy_input_value == 0 | energy_input_value / value > max_energy_IOcoef) %>%
      mutate(energy_input_value = if_else(energy_input_value == 0, value * min_energy_IOcoef, value * max_energy_IOcoef),
             sector = paste0("in_industry_", energy_input_sector)) %>%
      filter(energy_input_value > 0) %>%
      select(GCAM_region_ID, sector, fuel = energy_input_fuel, year, energy_input_value)

    # Replace these values back to the original energy balances table
    L1011.en_bal_EJ_R_Si_Fi_Yh <- L1011.en_bal_EJ_R_Si_Fi_Yh %>%
      left_join(L1011.in_EJ_R_rsrcFos_Yh_revised, by = c("GCAM_region_ID", "sector", "fuel", "year")) %>%
      mutate(value = if_else(is.na(energy_input_value), value, energy_input_value)) %>%
      select(-energy_input_value)

    # Next, revise the USA corn ethanol volume in 2015 to match EPA's estimate
    L1011.en_bal_EJ_R_Si_Fi_Yh$value[L1011.en_bal_EJ_R_Si_Fi_Yh$GCAM_region_ID == 1 &
                                       L1011.en_bal_EJ_R_Si_Fi_Yh$year == 2015 &
                                       L1011.en_bal_EJ_R_Si_Fi_Yh$sector == "in_trn_road" &
                                       L1011.en_bal_EJ_R_Si_Fi_Yh$fuel == "refined biofuels_ethanol"] <- 1.123
