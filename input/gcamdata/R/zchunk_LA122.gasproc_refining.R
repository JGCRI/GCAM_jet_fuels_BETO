...
module_energy_LA122.gasproc_refining <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    EcYield_kgm2_hi <- EcYield_kgm2_lo <- GCAM_commodity <- GCAM_region_ID <- GLU <-
      ...
      IOcoef <- rev_coef <- out_value <- coef <- SecOutRatio <- rev_output.ratio <-
      gross_exports <- gross_imports <- net_trade <- NULL # silence package check notes
    ...
    otaq_biod_adj<-get_data(all_data, "energy/OTAQ_adj_bio") %>%
      gather_years() %>%
      filter(sector %in% c("biodiesel")) %>%
      mutate(biomassOil_tech="Soybean oil",
             GCAM_region_ID=1) %>%
      rename(value_otaq=value,
             technology=sector) %>%
    ...
    # Remove "x" year
    A22.globaltech_coef %>%
      semi_join(select(calibrated_techs, supplysector, subsector, technology),
                by = c("supplysector", "subsector", "technology")) %>%
      left_join(select(calibrated_techs, supplysector, subsector, technology, minicam.energy.input, sector, fuel),
    ...
    # BIOMASS LIQUIDS: Ethanol and biodiesel output are equal to regional TPES plus net exports
    ...
    A_biofuel_types_R <- gather(select(A_regions, GCAM_region_ID, region, ethanol, biodiesel, biomassOil_tech),
      ...
      # Add feedstock-specific technologies
      left_join(bio_feed_mapping %>% select(technology,biomassOil_tech), by = "biomassOil_tech") %>%
      mutate(technology = if_else(grepl("ethanol",Biofuel),default_technology,technology),
      ...
      mutate(technology = if_else(is.na(technology), default_technology, technology),
             biomassOil_tech = if_else(Biofuel == "biodiesel" & !is.na(GCAM_commodity), GCAM_commodity, biomassOil_tech),
    ...
    BIOMASS_LIQUIDS <- tibble(fuel = c("refined biofuels_ethanol", "refined biofuels_FT"),
                              Biofuel = c("ethanol", "biodiesel"))

    L1012.en_bal_EJ_R_Si_Fi_Yh %>%
      filter(sector == "TPES",
             fuel %in% BIOMASS_LIQUIDS$fuel) %>%
      left_join_error_no_match(BIOMASS_LIQUIDS, by = "fuel") %>%
      left_join_error_no_match(L1262.Biofuel_GrossTrade_EJ_R_C_Y,
                               by = c("GCAM_region_ID", Biofuel = "GCAM_commodity", "year")) %>%
      left_join(A_biofuel_types_R %>% select(-biomassOil_tech), by = c("GCAM_region_ID", "Biofuel")) %>%
      mutate(value = (value + net_trade) * share) %>%
      select(GCAM_region_ID, technology, year, value) -> L122.out_EJ_R_biofuel_Yh


    # HACK JS: 01/2021. Biodiesel and corn ethanol values calibrate from IEA balances are larger than the values provided by OTAQ in 2015.
    # We create new categories to test if it works (test what happens in the future)

    L122.out_EJ_R_biofuel_Yh.usa <- L122.out_EJ_R_biofuel_Yh %>%
      filter(GCAM_region_ID==1,
             grepl("biodiesel",technology),
             year>=2005) %>%
      left_join_error_no_match(otaq_biod_adj, by=c("GCAM_region_ID","technology","year")) %>%
      mutate(value_other=value-value_otaq)

    L122.out_EJ_R_biofuel_Yh.usa.otaq <- L122.out_EJ_R_biofuel_Yh.usa %>%
      select(GCAM_region_ID,technology,biomassOil_tech,year,value_otaq) %>%
      rename(value=value_otaq) %>%
      select(-biomassOil_tech)

    L122.out_EJ_R_biofuel_Yh.usa.other <- L122.out_EJ_R_biofuel_Yh.usa %>%
      select(GCAM_region_ID,technology,biomassOil_tech,year,value_other) %>%
      rename(value=value_other) %>%
      mutate(technology=if_else(technology=="soybean biodiesel","other biodiesel",technology),
             biomassOil_tech=if_else(biomassOil_tech=="Soybean oil","other biodiesel",biomassOil_tech))

    L122.out_EJ_R_biofuel_Yh <- L122.out_EJ_R_biofuel_Yh %>%
      filter(!(GCAM_region_ID==1 & technology %in% c("soybean biodiesel") & year>=2005)) %>%
      bind_rows(L122.out_EJ_R_biofuel_Yh.usa.otaq)


    # Create the fixed output table for other biodiesel
    # Expand to ll GCAM years
    StubTechFixOut_otherBiod <- L122.out_EJ_R_biofuel_Yh.usa.other %>%
      filter(year==2015) %>%
      select(-year) %>%
      repeat_add_columns(tibble(year=MODEL_FUTURE_YEARS)) %>%
      bind_rows(L122.out_EJ_R_biofuel_Yh.usa.other) %>%
      arrange(year)

    L122.StubTechFixOut_otherBiod <- StubTechFixOut_otherBiod %>%
      mutate(region="USA",
      ...
      select(region,supplysector,subsector,stub.technology,year,fixedOutput,share.weight.year,share.weight)
    ...
    L122.out_EJ_R_oilrefining_Yh %>%
      ...
      select(-value_en_bal, -value_gtlctl) -> L122.out_EJ_R_IEA_oilrefining_Yh
    ...
    #With the addition of regional refined liquids, the inputs must be scaled to match the new production amounts.
    #To approximate this, add each regions net_exports of refined liquids to its oil input
    ...
    # Note these values have several non-thermodynamically feasible values because oil in is assumed
    # to follow TPES from IEA while production is based off of TPES(IEA) + net_trade(Comtrade)
    L122.in_EJ_R_oilrefining_F_Yh %>%
    ...
    adj_calibrated_techs<-calibrated_techs %>%
      select(sector,fuel) %>%
      left_join(bio_feed_mapping %>% select(technology,fuel), by = "fuel") %>%
      mutate(sector = if_else(is.na(technology),sector,technology)) %>%
      select(-technology)

    L122.out_EJ_R_biofuel_Yh_rev <- L122.out_EJ_R_biofuel_Yh %>%
      rename(sector = technology) %>%
      left_join_keep_first_only(adj_calibrated_techs, by = "sector") %>%
    ...
    L122.out_EJ_R_IEA_refining_F_Yh <- bind_rows(L122.out_EJ_R_IEA_oilrefining_Yh, L122.out_EJ_R_gtlctl_Yh, L122.out_EJ_R_biofuel_Yh_rev)
    ...
    L122.out_EJ_R_refining_F_Yh %>%
      ...
      add_units("EJ") %>%
      add_comments("Combines all calibrated refinery output tables, including oil refining, gtl-ctl and biofuels. This assumes regions trade consistent with Comtrade data") %>%
    ...
    L122.out_EJ_R_IEA_refining_F_Yh %>%
      add_title("Outputs of refining by GCAM region / fuel / historical year") %>%
      add_units("EJ") %>%
      add_comments("Combines all calibrated refinery output tables, including oil refining, gtl-ctl and biofuels. This assumes each region consumes all the refining it produces.") %>%
      add_legacy_name("L122.out_EJ_R_refining_F_Yh") %>%
      add_precursors("common/GCAM_region_names", "energy/calibrated_techs",
                     "energy/A_regions", "energy/A21.globaltech_coef", "energy/A22.globaltech_coef", "L1012.en_bal_EJ_R_Si_Fi_Yh",
                     "L121.in_EJ_R_unoil_F_Yh")  ->
    ...
    L122.in_Mt_R_C_Yh %>%
      ...
      add_precursors("common/GCAM_region_names",  "energy/calibrated_techs",
                     ...
                     "L1262.Biofuel_GrossTrade_EJ_R_C_Y","energy/bio_feed_mapping") ->
      L122.in_Mt_R_C_Yh
    ...
    L122.StubTechFixOut_otherBiod %>%
      ...
      add_precursors("L122.out_EJ_R_refining_F_Yh","energy/OTAQ_adj_bio") ->
      L122.StubTechFixOut_otherBiod
